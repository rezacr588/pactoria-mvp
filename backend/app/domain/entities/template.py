...
class LegalTemplate(AggregateRoot[TemplateId]):
    ...
    def create(cls,
               ...
               content: str,
               created_by_user_id: str,
               description: Optional[str] = None) -> 'LegalTemplate':
    ...
    def submit_for_legal_review(self, user_id: str):
        ...
        if self._status != TemplateStatus.DRAFT:
            raise BusinessRuleViolationError("Only draft templates can be submitted for review")
        
        self._status = TemplateStatus.UNDER_REVIEW
        self._increment_version()
    
    def complete_legal_review(self, 
                             reviewer_id: str, 
                             approved: bool, 
    ...
    def deprecate(self, 
        ...
        if self._status not in [TemplateStatus.ACTIVE, TemplateStatus.APPROVED]:
            raise BusinessRuleViolationError("Only active or approved templates can be deprecated")
    ...
    def record_usage(self, success: bool, customization_time_minutes: float):
        ...
        if self._usage_count == 1:
            ...
            total_time = self._average_customization_time_minutes * (self._usage_count - 1)
    ...
    def is_suitable_for_business(self, 
                                ...
                                company_size: CompanySize) -> bool:
        """Check if template is suitable for a specific business"""
        industry_suitable = (
            not self._suitable_for_industries or 
            industry in self._suitable_for_industries
        )
        
        size_suitable = (
            not self._suitable_for_company_sizes or 
            company_size in self._suitable_for_company_sizes
        )
        ...
        for variable in self._variables:
            ...
            if value is not None:
                ...
                if variable.choices and value not in variable.choices:
                    errors.append(f"Variable '{variable.display_name}' must be one of: {variable.choices}")
    ...
    def can_be_used(self) -> bool:
        ...
        return (
            self._status == TemplateStatus.ACTIVE and
            self._is_active and
    ...
    def _validate_name(self, name: str):
        ...
        if len(name.strip()) > 200:
            raise DomainValidationError("Template name too long (max 200 characters)", "name", name)
    
    def _validate_content(self, content: str):
        """Validate template content"""
        if not content or not content.strip():
            raise DomainValidationError("Template content cannot be empty", "content", content)
        
        if len(content) < 100:
            raise DomainValidationError("Template content too short (min 100 characters)", "content", content)
