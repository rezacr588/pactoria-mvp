...
class ProductionSafetyMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if hasattr(settings, 'PRODUCTION_CONNECTION_WARNING') and settings.PRODUCTION_CONNECTION_WARNING:
            response.headers["X-Production-Warning"] = "true"
            response.headers["X-Connection-Mode"] = "local-to-production"
            response.headers["X-Data-Warning"] = "All changes are permanent in production"
            ...
            if request.method in ["POST", "PUT", "PATCH", "DELETE"]:
                ...
                response.headers["X-Data-Modification-Warning"] = "PRODUCTION data modified"
