...
IN WITNESS WHEREOF, the parties have executed this Agreement.
    ...
    def test_complete_user_registration_to_contract_generation(
        self, client, company_admin_user, mock_ai_responses
    ):
        """
        ...
        with next(mock_ai_responses):
            ...
            registration_response = client.post(
                "/api/v1/auth/register", json=company_admin_user
            )
            ...
            generate_response = client.post(
                f"/api/v1/contracts/{contract_id}/generate", headers=auth_headers
            )
            ...
            update_data = {"title": "Updated Professional Consulting Agreement"}
    ...
    def test_multi_user_company_workflow(self, client):
        ...
        with patch("app.api.v1.endpoints.auth.MOCK_USERS") as mock_users:
            ...
            company_users_response = client.get(
                "/api/v1/auth/company/users", headers=admin_headers
            )
    ...
    def test_template_recommendation_to_generation_workflow(
        self, client, company_admin_user
    ):
        """
        ...
        with patch("app.api.v1.endpoints.ai.ai_service") as mock_ai_service:
            ...
            with patch("app.api.v1.endpoints.contracts.ai_service") as mock_contract_ai:
                mock_contract_ai.generate_contract = AsyncMock(
                    return_value=(
                        "IT SERVICES AGREEMENT\n\nCustom software development contract...",
                        {"processing_time_ms": 2000},
                    )
                )
                mock_contract_ai.validate_contract_compliance = AsyncMock(
                    return_value={"overall_score": 0.94, "risk_score": 3}
                )
    ...
    def test_contract_lifecycle_status_transitions(
        self, client, company_admin_user, mock_ai_responses
    ):
        """
    ...
    @pytest.mark.parametrize(
        "subscription_tier,max_contracts",
        [("free_trial", 10), ("starter", 50), ("professional", 200)],
    )
    def test_subscription_limits_workflow(
        self, client, company_admin_user, subscription_tier, max_contracts
    ):
        """
        ...
        assert company_data["subscription_tier"] in [
            "free_trial",
            "starter",
            "professional",
            "enterprise",
    ...
    def test_mvp_performance_requirements_e2e(
        self, client, company_admin_user, mock_ai_responses
    ):
        """
