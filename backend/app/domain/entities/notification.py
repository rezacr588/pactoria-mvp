...
class Notification(AggregateRoot[NotificationId]):
    ...
    def __init__(self,
        ...
        self._expires_at = expires_at or (datetime.now(timezone.utc) + timedelta(days=30))
    ...
    def schedule_for(self, scheduled_time: datetime):
        ...
        if scheduled_time <= datetime.now(timezone.utc):
            raise BusinessRuleViolationError("Cannot schedule notification for past time")
        
        if scheduled_time > self._expires_at:
            raise BusinessRuleViolationError("Cannot schedule notification after expiry time")
    ...
    def cancel_scheduled(self):
        ...
        if self._status == NotificationStatus.SENT:
            raise BusinessRuleViolationError("Cannot cancel notification that has already been sent")
    ...
    def mark_as_sent(self):
        ...
        if self._status != NotificationStatus.PENDING:
            raise BusinessRuleViolationError("Can only mark pending notifications as sent")
    ...
    def mark_as_read(self, user_id: str):
        ...
        if self._status not in [NotificationStatus.DELIVERED, NotificationStatus.SENT]:
            raise BusinessRuleViolationError("Can only mark delivered notifications as read")
    ...
    def can_be_sent(self) -> bool:
        ...
        return (
            self._status == NotificationStatus.PENDING and
            not self.is_expired() and
            (self._scheduled_for is None or self._scheduled_for <= datetime.now(timezone.utc))
        )
    
    def should_retry(self) -> bool:
        """Check if notification should be retried"""
        return (
            self._status in [NotificationStatus.PENDING, NotificationStatus.SENT] and
            self._failed_deliveries > 0 and
            len(self._delivery_attempts) < self._max_retry_attempts and
            not self.is_expired()
        )
    ...
    def get_next_retry_time(self) -> Optional[datetime]:
        ...
        retry_index = min(len(self._delivery_attempts), len(self._retry_delay_minutes) - 1)
    ...
    def _validate_subject(self, subject: str):
        ...
        if not subject or not subject.strip():
            raise DomainValidationError("Notification subject cannot be empty", "subject", subject)
        
        if len(subject.strip()) > 200:
            raise DomainValidationError("Notification subject too long (max 200 characters)", "subject", subject)
    
    def _validate_content(self, content: str):
        """Validate notification content"""
        if not content or not content.strip():
            raise DomainValidationError("Notification content cannot be empty", "content", content)
    
    def _validate_recipients(self, recipients: List[NotificationRecipient]):
        """Validate notification recipients"""
        if not recipients:
            raise DomainValidationError("Notification must have at least one recipient", "recipients", recipients)
        
        if len(recipients) > 1000:  # Reasonable batch limit
            raise DomainValidationError("Too many recipients (max 1000)", "recipients", recipients)
