...
class TestAuthenticationErrorScenarios:
    ...
    def test_invalid_token_scenarios(self, e2e_test_base: E2ETestBase, error_scenarios):
        ...
        for scenario in invalid_auth_scenarios:
            ...
            if response.status_code == 401:
                ...
                assert (
                    "Unauthorized" in error_data["detail"]
                    or "Authentication" in error_data["detail"]
                )
    ...
    def test_expired_token_scenarios(self, e2e_test_base: E2ETestBase):
        ...
        expired_token = create_access_token(
            subject=user.id, expires_delta=timedelta(seconds=-1)
        )
    ...
    def test_insufficient_permissions_scenarios(self, e2e_test_base: E2ETestBase):
        ...
        bulk_data = {"updates": [{"contract_id": "fake-id", "status": "active"}]}

        # Viewer should be forbidden for bulk operations
        bulk_response = client.post(
            "/api/v1/bulk/contracts/update", json=bulk_data, headers=viewer_headers
        )
...
class TestValidationErrorScenarios:
    ...
    def test_contract_validation_errors(
        ...
        for error_case in validation_errors:
            ...
            response = client.post(
                "/api/v1/contracts/", json=test_data, headers=headers
            )
    ...
    def test_file_upload_validation_errors(self, e2e_test_base: E2ETestBase):
        ...
        files = {
            "file": ("test.exe", io.BytesIO(b"executable"), "application/octet-stream")
        }
    ...
    def test_search_validation_errors(self, e2e_test_base: E2ETestBase):
        ...
        invalid_type_response = client.get(
            "/api/v1/contracts/?contract_type=invalid_type", headers=headers
        )
        ...
        invalid_status_response = client.get(
            "/api/v1/contracts/?status=invalid_status", headers=headers
        )
        ...
        invalid_page_responses = [
            client.get(
                "/api/v1/contracts/?page=0", headers=headers
            ),  # Page must be >= 1
            client.get("/api/v1/contracts/?page=-1", headers=headers),
            client.get(
                "/api/v1/contracts/?size=0", headers=headers
            ),  # Size must be > 0
            client.get(
                "/api/v1/contracts/?size=1001", headers=headers
            ),  # Size too large
        ]
        ...
        invalid_search_data = {"query": "test", "filters": {"invalid_field": "value"}}

        invalid_search_response = client.post(
            "/api/v1/search/contracts", json=invalid_search_data, headers=headers
        )
    ...
    def test_bulk_operation_validation_errors(self, e2e_test_base: E2ETestBase):
        ...
        empty_response = client.post(
            "/api/v1/bulk/contracts/update", json=empty_bulk_data, headers=headers
        )
        e2e_test_base.assert_response_error(empty_response, 422)

        # Test 2: Bulk update with non-existent contract ID
        invalid_bulk_data = {
            "updates": [{"contract_id": "non-existent-id", "status": "active"}]
        }

        invalid_response = client.post(
            "/api/v1/bulk/contracts/update", json=invalid_bulk_data, headers=headers
        )
        e2e_test_base.assert_response_success(
            invalid_response
        )  # Should return partial success/failure info

        result = invalid_response.json()
        assert "failed_count" in result or "errors" in result

        # Test 3: Bulk export with invalid format
        invalid_export_data = {"contract_ids": [], "format": "invalid_format"}

        invalid_export_response = client.post(
            "/api/v1/bulk/contracts/export", json=invalid_export_data, headers=headers
        )
...
class TestBusinessLogicErrorScenarios:
    ...
    def test_contract_state_transition_errors(self, e2e_test_base: E2ETestBase):
        ...
        completed_contract = e2e_test_base.create_test_contract(
        ...
        if invalid_transition_response.status_code == 400:
            ...
            assert (
                "transition" in error_data["detail"].lower()
                or "status" in error_data["detail"].lower()
            )
    ...
    def test_company_isolation_violations(self, e2e_test_base: E2ETestBase):
        ...
        files = {
            "file": (
                "company_a_file.txt",
                io.BytesIO(b"Company A content"),
                "text/plain",
            )
        }
        upload_response = client.post(
            "/api/v1/files/upload", files=files, headers=headers_a
        )
        ...
        cross_file_access = client.get(
            f"/api/v1/files/{file_a['file_id']}", headers=headers_b
        )
    ...
    def test_concurrent_operation_conflicts(self, e2e_test_base: E2ETestBase):
        ...
        response1 = client.put(
            f"/api/v1/contracts/{contract.id}", json=update1_data, headers=headers1
        )
        response2 = client.put(
            f"/api/v1/contracts/{contract.id}", json=update2_data, headers=headers2
        )
        ...
        update_response = client.put(
            f"/api/v1/contracts/{another_contract.id}",
            json=update_data,
            headers=headers1,
        )
        delete_response = client.delete(
            f"/api/v1/contracts/{another_contract.id}", headers=headers2
        )
...
class TestExternalServiceErrorScenarios:
    ...
    def test_ai_service_failure_scenarios(self, e2e_test_base: E2ETestBase):
        ...
        with patch(
            "app.services.ai_service.ai_service.generate_contract"
        ) as mock_generate:
        ...
        with patch(
            "app.services.ai_service.ai_service.analyze_compliance"
        ) as mock_analyze:
...
class TestRateLimitingAndSecurity:
    ...
    def test_rate_limiting_scenarios(self, e2e_test_base: E2ETestBase):
        ...
        if rate_limited_responses:
            ...
            assert "detail" in error_data
            assert (
                "rate" in error_data["detail"].lower()
                or "limit" in error_data["detail"].lower()
            )
    ...
    def test_malicious_input_scenarios(self, e2e_test_base: E2ETestBase):
        ...
        sql_injection_response = client.get(
            f"/api/v1/contracts/?search={malicious_search}", headers=headers
        )
        e2e_test_base.assert_response_success(
        ...
        xss_response = client.post(
            "/api/v1/contracts/", json=xss_contract_data, headers=headers
        )
        ...
        large_payload_response = client.post(
            "/api/v1/contracts/", json=large_json, headers=headers
        )
    ...
    def test_brute_force_protection(self, e2e_test_base: E2ETestBase):
        ...
        for i in range(20):  # Many login attempts with wrong password
            login_data = {"email": user.email, "password": f"wrong_password_{i}"}
...
class TestDataIntegrityAndRecovery:
    ...
    def test_system_consistency_under_errors(self, e2e_test_base: E2ETestBase):
        ...
        failed_create_response = client.post(
            "/api/v1/contracts/", json=invalid_contract_data, headers=headers
        )
        ...
        success_response = client.post(
            "/api/v1/contracts/", json=valid_contract_data, headers=headers
        )
