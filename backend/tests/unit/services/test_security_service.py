...
class TestSecurityMonitor:
    ...
    def test_analyze_request_suspicious_url(
        self, security_monitor_instance, mock_request
    ):
        """Test analyzing request with suspicious URL"""
        mock_request.url.__str__ = Mock(
            return_value="https://example.com/api/test?id=1' OR 1=1"
        )
    ...
    def test_analyze_request_suspicious_user_agent(
        self, security_monitor_instance, mock_request
    ):
    ...
    def test_analyze_request_multiple_forwarding_headers(
        self, security_monitor_instance, mock_request
    ):
        ...
        result = security_monitor_instance.analyze_request(mock_request)
        assert result is not None
        assert "Multiple forwarding headers" in str(
    ...
    def test_check_rate_limit_exceeded(self, security_monitor_instance, mock_request):
        ...
        with patch.object(
            security_monitor_instance.rate_limiter, "is_allowed", return_value=False
        ):
    ...
    def test_log_failed_login_brute_force_detection(self, security_monitor_instance):
        ...
        brute_force_events = [
            event
            for event in security_monitor_instance.security_events
...
class TestSecurityMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_security_middleware_allows_normal_request(
        ...
        with patch.object(
            security_middleware.monitor, "check_rate_limit", return_value=True
        ), patch.object(
            security_middleware.monitor, "analyze_request", return_value=None
        ):
    ...
    @pytest.mark.asyncio
    async def test_security_middleware_blocks_rate_limited_request(
        self, security_middleware, mock_request
    ):
        ...
        with patch.object(
            security_middleware.monitor, "check_rate_limit", return_value=False
        ):
    ...
    @pytest.mark.asyncio
    async def test_security_middleware_logs_suspicious_request(
        self, security_middleware, mock_request
    ):
        """Test middleware logs suspicious request but allows it"""
        suspicious_event = SecurityEvent(
            "suspicious_request", "medium", {"test": "data"}
        )
        ...
        with patch.object(
            ...
            security_middleware.monitor,
            "analyze_request",
            return_value=suspicious_event,
        ):
...
class TestSecurityPatterns:
    ...
    def test_pattern_detection_case_insensitive(self, monitor, mock_request):
        ...
        mock_request.url.__str__ = Mock(
            return_value="https://example.com/test?id=1' or 1=1"
        )
