...
from app.core.responses import SuccessResponse, PaginatedResponse, ResponseFormatter
...
class TestPaginatedResponse:
    ...
    def test_paginated_response_create_single_page(self):
        ...
        items = ["item1", "item2", "item3"]
        response = PaginatedResponse.create(items=items, total=3, page=1, size=10)
    ...
    def test_paginated_response_create_first_page(self):
        ...
        items = list(range(10))
        response = PaginatedResponse.create(items=items, total=25, page=1, size=10)
    ...
    def test_paginated_response_create_last_page(self):
        ...
        response = PaginatedResponse.create(items=items, total=25, page=3, size=10)
    ...
    def test_paginated_response_page_calculation(self):
        ...
        # Test exact division
        response = PaginatedResponse.create(
            items=list(range(10)), total=20, page=1, size=10
        )
        assert response.pages == 2

        # Test with remainder
        response = PaginatedResponse.create(
            items=list(range(10)), total=21, page=1, size=10
        )
        assert response.pages == 3

        # Test single item
        response = PaginatedResponse.create(items=["item1"], total=1, page=1, size=10)
        assert response.pages == 1
