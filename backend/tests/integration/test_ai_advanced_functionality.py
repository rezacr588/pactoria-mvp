...
class TestAdvancedAIFunctionality:
        ...
        return {
            "multi_party_agreement": {
                ...
                "complexity_indicators": [
                    "three parties", "revenue sharing", "different obligations", 
                    "ip obligations", "gdpr", "18 months", "£200,000"
                ]
            },
            "international_services": {
                ...
                """,
                "complexity_indicators": [
                    "multiple jurisdictions", "gdpr", "ccpa", "data analytics",
                    "cross-border", "multiple currencies", "different notice periods"
                ]
            },
            "high_risk_compliance": {
                ...
                """,
                "complexity_indicators": [
                    "financial services", "fca compliance", "pci dss", "liability caps",
                    "professional indemnity", "10 years", "background checks",
                    "zero-tolerance", "regulatory breaches"
                ]
            }
        }
    ...
    @pytest.mark.asyncio
    async def test_multi_party_contract_generation(self, ai_service, complex_contract_scenarios):
        ...
        assert len(response.content) > 1000, "Multi-party agreement should be substantial"
        ...
        assert complexity_score >= 0.70, f"Multi-party complexity score {complexity_score:.2%} too low"
    ...
    @pytest.mark.asyncio
    async def test_international_compliance_handling(self, ai_service, complex_contract_scenarios):
        ...
        compliance_indicators = ["gdpr", "data protection", "jurisdiction", "cross-border"]
        found_compliance = sum(1 for indicator in compliance_indicators if indicator in content_lower)
        assert found_compliance >= 2, "Should address international compliance"
        
        # Currency handling
        currencies = ["gbp", "eur", "usd", "pounds", "euros", "dollars"]
        found_currencies = sum(1 for currency in currencies if currency in content_lower)
        ...
        found_jurisdictions = sum(1 for jurisdiction in jurisdictions if jurisdiction in content_lower)
        assert found_jurisdictions >= 2, "Should show multi-jurisdictional awareness"

    @pytest.mark.asyncio
    async def test_high_risk_regulatory_compliance(self, ai_service, complex_contract_scenarios):
        ...
        assert len(response.content) > 1500, "High-risk contract should be comprehensive"
        
        # Check regulatory compliance elements
        regulatory_terms = ["compliance", "regulatory", "fca", "liability", "insurance", "audit"]
        found_regulatory = sum(1 for term in regulatory_terms if term in content_lower)
        assert found_regulatory >= 4, f"Should address regulatory requirements (found {found_regulatory})"
        
        # Professional standards
        professional_terms = ["professional", "indemnity", "standards", "background check"]
        found_professional = sum(1 for term in professional_terms if term in content_lower)
        assert found_professional >= 2, "Should address professional standards"

    @pytest.mark.asyncio
    async def test_minimal_vs_verbose_contract_adaptation(self, ai_service, edge_case_contracts):
        ...
        assert len(verbose_response.content) > len(minimal_response.content), \
        ...
        for content in [minimal_lower, verbose_lower]:
            found_essentials = sum(1 for essential in legal_essentials if essential in content)
            assert found_essentials >= 2, "Contract should contain legal essentials"

    @pytest.mark.asyncio
    async def test_ambiguous_requirements_handling(self, ai_service, edge_case_contracts):
        ...
        assert len(response.content) > 200, "Should produce substantial content despite ambiguity"
        
        # Should include standard contract elements
        content_lower = response.content.lower()
        standard_elements = ["services", "payment", "term", "termination", "governing law"]
        found_elements = sum(1 for element in standard_elements if element in content_lower)
        assert found_elements >= 3, "Should include standard elements despite ambiguous input"
    ...
    async def test_concurrent_ai_requests_performance(self, ai_service):
        ...
        assert total_time < 60000, f"Concurrent requests took {total_time:.0f}ms (over 60s)"
        ...
        for case_name, contract_content in edge_cases.items():
            ...
            try:
                ...
                if case_name in ["single_sentence", "non_english_elements"]:
                    assert response.overall_score < 0.7, \
    ...
    async def test_contract_size_scaling(self, ai_service):
        ...
        size_tests = [
            ...
            ("medium", "Professional services agreement with standard clauses including payment, confidentiality, and termination", 800),
            ("large", "Comprehensive professional services agreement with detailed scope, payment schedules, intellectual property clauses, confidentiality provisions, termination procedures, dispute resolution, governing law, and extensive schedules", 2000)
        ]
        
        for size_name, prompt, max_tokens in size_tests:
            request = ContractGenerationRequest(
                plain_english_input=prompt,
                contract_type="service_agreement"
            )
            ...
            if size_name == "small":
                assert 200 <= len(response.content) <= 1500, \
                    f"Small contract length {len(response.content)} not appropriate"
            elif size_name == "medium":
                assert 500 <= len(response.content) <= 4000, \
    ...
    async def test_specialized_contract_types_accuracy(self, ai_service):
        ...
        for contract_type, description in specialized_contracts.items():
            request = ContractGenerationRequest(
                plain_english_input=description,
            ...
            type_specific_terms = {
                ...
                "supplier_agreement": ["supplier", "goods", "quality", "delivery", "specifications"],
                "terms_conditions": ["terms", "conditions", "user", "service", "website"]
            }
    ...
    async def test_ai_context_awareness(self, ai_service):
        ...
        tech_terms = ["blockchain", "smart contract", "cryptocurrency", "defi", "security audit"]
        found_tech = sum(1 for term in tech_terms if term in content_lower)
        assert found_tech >= 2, f"Should show tech context awareness (found {found_tech} terms)"
    ...
    async def test_error_recovery_and_fallbacks(self, ai_service):
        ...
        for problematic_input in problematic_inputs:
            request = ContractGenerationRequest(
                plain_english_input=problematic_input,
                contract_type="service_agreement"
            )
            ...
            try:
                ...
                found_malicious = sum(1 for pattern in malicious_patterns if pattern in content_lower)
    ...
    async def test_temperature_consistency_analysis(self, ai_service):
        ...
        def measure_consistency(responses):
            ...
            for response in responses:
                ...
                all_words.extend([w for w in words if len(w) > 3])  # Only meaningful words
        ...
        assert low_consistency >= high_consistency - 0.1, \
