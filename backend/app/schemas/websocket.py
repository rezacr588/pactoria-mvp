...
class ConnectionMessage(WebSocketMessage):
    ...
    def disconnected(cls, user_id: str, company_id: str):
        return cls(status="disconnected", user_id=user_id, company_id=company_id)
...
class ContractUpdateMessage(WebSocketMessage):
    ...
    updated_by_name: str = Field(
        ..., description="Full name of user who made the update"
    )
...
class NotificationMessage(WebSocketMessage):
    ...
    target_user_id: Optional[str] = Field(
        None, description="Target user ID (if user-specific)"
    )
    target_role: Optional[str] = Field(
        None, description="Target role (if role-specific)"
    )
...
class SystemMessage(WebSocketMessage):
    ...
    affects_all_users: bool = Field(
        True, description="Whether message affects all users"
    )
    maintenance_mode: bool = Field(
        False, description="Whether system is in maintenance"
    )
    estimated_duration: Optional[str] = Field(
        None, description="Estimated duration for maintenance"
    )
...
class AlertMessage(WebSocketMessage):
    ...
    category: str = Field(
        ..., description="Alert category (SECURITY, COMPLIANCE, SYSTEM, BUSINESS)"
    )
    affected_contracts: Optional[List[str]] = Field(
        None, description="List of affected contract IDs"
    )
    action_required: bool = Field(
        False, description="Whether immediate action is required"
    )
    auto_dismiss: bool = Field(False, description="Whether alert auto-dismisses")
    dismiss_after: Optional[int] = Field(
        None, description="Auto-dismiss time in seconds"
    )
...
class UserPresenceMessage(WebSocketMessage):
    ...
    status: str = Field(
        ..., description="Presence status (online, offline, away, busy)"
    )
...
class LiveUpdateMessage(WebSocketMessage):
    ...
    cursor_position: Optional[int] = Field(
        None, description="Cursor position for text fields"
    )
...
class BatchMessage(WebSocketMessage):
    ...
    messages: List[WebSocketMessage] = Field(
        ..., description="List of messages in batch"
    )
