...
class AdvancedSearchService:
    ...
    async def search_contracts(
        self, request: ContractSearchRequest, current_user: User
    ) -> ContractSearchResults:
        """
    ...
    async def search_users(
        self, request: UserSearchRequest, current_user: User
    ) -> UserSearchResults:
        """
        ...
        try:
            ...
            # Convert to search results
            items = [self._user_to_search_result(user, context) for user in users]
    ...
    async def search_templates(
        self, request: TemplateSearchRequest, current_user: User
    ) -> TemplateSearchResults:
        """
    ...
    def _apply_contract_filters(self, query: Query, filters) -> Query:
        ...
        if filters.contract_type:
            type_enums = [
                ContractType(ctype.lower()) for ctype in filters.contract_type
            ]
        ...
        if filters.supplier_name:
            query = query.filter(
                Contract.supplier_name.ilike(f"%{filters.supplier_name}%")
            )

        if filters.contract_value:
            query = self._apply_numeric_filter(
                query, Contract.contract_value, filters.contract_value
            )

        if filters.start_date:
            query = self._apply_date_filter(
                query, Contract.start_date, filters.start_date
            )
        ...
        if filters.created_at:
            query = self._apply_date_filter(
                query, Contract.created_at, filters.created_at
            )

        if filters.updated_at:
            query = self._apply_date_filter(
                query, Contract.updated_at, filters.updated_at
            )
    ...
    def _apply_user_filters(self, query: Query, filters) -> Query:
        ...
        if filters.role:
            role_enums = [
                UserRole(role.lower())
                for role in filters.role
                if role.lower() in [r.value for r in UserRole]
            ]
        ...
        if filters.last_login_at:
            query = self._apply_date_filter(
                query, User.last_login_at, filters.last_login_at
            )
    ...
    def _apply_date_filter(
        self, query: Query, column, date_filter: DateRangeFilter
    ) -> Query:
    ...
    def _apply_numeric_filter(
        self, query: Query, column, numeric_filter: NumericRangeFilter
    ) -> Query:
    ...
    def _contract_to_search_result(
        self, contract: Contract, context: SearchContext
    ) -> ContractSearchResult:
        ...
        if context.highlight and context.query:
            result.highlights = self._generate_highlights(
                contract, context.query, "contract"
            )

        return result

    def _user_to_search_result(
        self, user: User, context: SearchContext
    ) -> UserSearchResult:
    ...
    def _template_to_search_result(
        self, template: Template, context: SearchContext
    ) -> TemplateSearchResult:
        ...
        if context.highlight and context.query:
            result.highlights = self._generate_highlights(
                template, context.query, "template"
            )

        return result

    def _generate_highlights(
        self, obj, query: str, obj_type: str
