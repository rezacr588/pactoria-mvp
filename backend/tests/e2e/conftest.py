...
class TestDataFactory:
    ...
    def create_contract_data(
        company_id: str, created_by: str, template_id: str = None
    ) -> Dict[str, Any]:
...
def mock_ai_service():
    ...
    mock.generate_contract = AsyncMock(
        return_value=Mock(
    ...
    mock.analyze_compliance = AsyncMock(
        return_value=Mock(
            overall_score=0.94,
            ...
            recommendations=[
                "Add GDPR Article 28 clauses",
                "Clarify IP ownership",
                "Define liability caps",
            ],
...
class E2ETestBase:
    ...
    def create_test_contract(
        self, company: Company, user: User, data: Dict[str, Any] = None
    ) -> Contract:
...
class WebSocketTestHelper:
    ...
    async def websocket_connection(
        self, token: str, endpoint: str = "/api/v1/ws/connect"
    ):
    ...
    async def send_message(
        self, websocket, message_type: str, data: Dict[str, Any] = None
    ):
...
class PerformanceTestHelper:
    ...
    def assert_performance(self, metric_name: str, max_time_ms: float):
        ...
        assert (
            avg_time <= max_time_ms
        ), f"Average {metric_name} time {avg_time:.2f}ms exceeds limit {max_time_ms}ms"
        assert (
            max_time <= max_time_ms * 2
        ), f"Max {metric_name} time {max_time:.2f}ms exceeds limit {max_time_ms * 2}ms"
...
def bulk_operation_data():
    ...
    return {
        ...
        "export_requests": [
            ...
            {
                "format": "excel",
                "fields": ["title", "contract_type", "created_at", "compliance_score"],
            },
...
def error_scenarios():
    ...
    return {
        ...
        "validation_errors": [
            ...
            {
                "data": {"contract_type": "invalid"},
                "field": "contract_type",
                "expected_status": 422,
            },
            {
                "data": {"plain_english_input": "x"},
