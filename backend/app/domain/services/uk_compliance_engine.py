...
class ComplianceAssessment:
    ...
    def is_compliant(self) -> bool:
        ...
        return self.overall_level in [ComplianceLevel.COMPLIANT, ComplianceLevel.MINOR_ISSUES]
    
    def has_critical_violations(self) -> bool:
        """Check if there are critical violations"""
        return any(v.severity == RiskLevel.CRITICAL for v in self.violations)
    
    def get_violations_by_severity(self, severity: RiskLevel) -> List[ComplianceViolation]:
...
class UKComplianceRuleEngine:
    ...
    def _initialize_uk_rules(self):
        ...
        self._add_rule(ComplianceRule(
            ...
            applicable_contract_types=[ContractType.SERVICE_AGREEMENT, ContractType.SUPPLIER_AGREEMENT]
        ))
        ...
        self._add_rule(ComplianceRule(
            ...
            applicable_company_sizes=[CompanySize.SMALL, CompanySize.MEDIUM, CompanySize.LARGE],
        ...
        self._add_rule(ComplianceRule(
            ...
            prohibited_clauses=["non_compete_excessive", "price_fixing", "market_sharing"],
    ...
    def get_applicable_rules(self, 
                           ...
                           frameworks: List[ComplianceFramework] = None) -> List[ComplianceRule]:
    ...
    def validate_contract(self, 
                         ...
                         company: Company,
                         contract_type: ContractType,
                         contract_value: Optional[Money] = None) -> ComplianceAssessment:
        ...
        for rule in applicable_rules:
            try:
                violation = self._validate_rule(rule, contract_content, company, contract_value)
                if violation:
        ...
        for framework in ComplianceFramework:
            framework_rules = [r for r in applicable_rules if framework in r.applicable_frameworks]
            if framework_rules:
                framework_passed = len([r for r in framework_rules if r.rule_id in passed_rules])
                framework_scores[framework.value] = (framework_passed / len(framework_rules)) * 100
        ...
        recommendations = self._generate_recommendations(violations, company, contract_type)
    ...
    def _validate_rule(self, 
                      ...
                      content: str, 
                      company: Company,
                      contract_value: Optional[Money]) -> Optional[ComplianceViolation]:
        ...
        if rule.validation_function:
            return self._execute_custom_validation(rule, content, company, contract_value)
    ...
    def _execute_custom_validation(self, 
                                 rule: ComplianceRule, 
                                 content: str, 
                                 company: Company,
                                 contract_value: Optional[Money]) -> Optional[ComplianceViolation]:
    ...
    def _validate_lawful_basis(self, rule: ComplianceRule, content: str) -> Optional[ComplianceViolation]:
    ...
    def _validate_reasonableness(self, 
        ...
        broad_exclusions = [
            "all liability", "any liability", "entire liability", 
            "total liability", "liability whatsoever"
        ]
        ...
        for exclusion in broad_exclusions:
            if exclusion in content_lower:
                ...
                if not any(term in content_lower for term in ["subject to", "except for", "save for"]):
                    return ComplianceViolation(
                        rule_id=rule.rule_id,
                        rule_title=rule.title,
                        severity=RiskLevel.HIGH,
    ...
    def _validate_consumer_fairness(self, 
                                  rule: ComplianceRule, 
                                  content: str, 
                                  company: Company) -> Optional[ComplianceViolation]:
        """Validate consumer fairness (B2C contracts)"""
        # Look for unfair terms indicators
        unfair_indicators = [
            "irrevocably", "without recourse", "no refund", "final sale",
            "exclude all warranties", "buyer beware"
        ]
    ...
    def _validate_director_authority(self, 
                                   rule: ComplianceRule, 
                                   content: str, 
                                   company: Company) -> Optional[ComplianceViolation]:
        """Validate director signing authority"""
        if company.company_type in [CompanyType.PRIVATE_LIMITED, CompanyType.PUBLIC_LIMITED]:
            authority_terms = ["duly authorized", "acting within authority", "board resolution"]
    ...
    def _validate_fca_authorization(self, 
                                  rule: ComplianceRule, 
                                  content: str, 
                                  company: Company) -> Optional[ComplianceViolation]:
        """Validate FCA authorization for financial services"""
        if company.industry == IndustryType.FINANCE:
            fca_terms = ["fca authorized", "financial conduct authority", "regulated activity"]
    ...
    def _validate_confidentiality_duration(self, 
                                         rule: ComplianceRule, 
                                         content: str) -> Optional[ComplianceViolation]:
    ...
    def _validate_eu_law_references(self, 
                                  rule: ComplianceRule, 
                                  content: str) -> Optional[ComplianceViolation]:
        """Validate EU law references post-Brexit"""
        eu_references = [
            "eu directive", "european directive", "eu regulation", "european regulation",
            "brussels regulation", "rome regulation"
        ]
    ...
    def _calculate_overall_score(self, violations: List[ComplianceViolation], total_rules: int) -> float:
        ...
        total_deductions = sum(violation_weights.get(v.severity, 10) for v in violations)
    ...
    def _generate_recommendations(self, 
        ...
        if critical:
            recommendations.append("URGENT: Address critical compliance violations before contract execution")
        
        # High-risk violations
        high = [v for v in violations if v.severity == RiskLevel.HIGH]
        if high:
            recommendations.append("Strongly recommend legal review for high-risk violations")
        
        # Industry-specific recommendations
        if company.industry == IndustryType.FINANCE and contract_type == ContractType.SERVICE_AGREEMENT:
            recommendations.append("Consider additional FCA regulatory requirements for financial services")
        
        if company.industry == IndustryType.HEALTHCARE:
            recommendations.append("Ensure compliance with healthcare-specific regulations")
        
        # Company size recommendations
        if company.company_size == CompanySize.MICRO:
            recommendations.append("Consider using simplified contract templates for micro businesses")
        
        # Contract type recommendations
        if contract_type == ContractType.EMPLOYMENT_CONTRACT:
            recommendations.append("Ensure all statutory employment rights are properly addressed")
    ...
    def suggest_clause_improvements(self, 
                                  content: str, 
                                  company: Company,
                                  contract_type: ContractType) -> List[str]:
        ...
        if company.is_vat_registered and "vat" not in content_lower:
            suggestions.append("Include VAT treatment clauses as company is VAT registered")
