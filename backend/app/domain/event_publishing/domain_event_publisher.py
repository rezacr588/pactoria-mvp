...
class DomainEventPublisher:
    ...
    def register_handler(self, event_type: str, handler: DomainEventHandler) -> None:
        ...
        self._logger.info(f"Registered handler {handler.__class__.__name__} for event type {event_type}")
    ...
    async def publish(self, event: DomainEvent) -> None:
        ...
        if not handlers:
            self._logger.debug(f"No handlers registered for event type {event.event_type}")
            return
        
        self._logger.info(f"Publishing event {event.event_type} to {len(handlers)} handlers")
    ...
    async def _handle_event_safely(self, handler: DomainEventHandler, event: DomainEvent) -> None:
