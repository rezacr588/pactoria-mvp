...
from app.domain.value_objects import (
    ContractStatus, ContractType, Money, DateRange, 
    Email, ContractParty, ComplianceScore, RiskAssessment
)
...
class TestContractEntity:
    ...
    def test_create_contract_with_valid_data(self):
        ...
        client = ContractParty("Client Corp", Email("client@example.com"), "Client Corp Ltd")
        supplier = ContractParty("Supplier Inc", Email("supplier@example.com"), "Supplier Inc Ltd")
    ...
    def test_create_contract_with_invalid_title_raises_error(self):
        ...
        # When & Then
        with pytest.raises(DomainValidationError, match="Contract title cannot be empty"):
            Contract.create(
                contract_id=contract_id,
                title=empty_title,
                contract_type=ContractType.SERVICE_AGREEMENT,
    ...
    def test_activate_contract_without_final_content_raises_error(self):
        ...
        # When & Then
        with pytest.raises(BusinessRuleViolationError, match="Cannot activate contract without final content"):
            contract.activate("user-123")
    ...
    def test_complete_contract_from_draft_status_raises_error(self):
        ...
        # When & Then
        with pytest.raises(BusinessRuleViolationError, match="Cannot complete contract that is not active"):
            contract.complete("user-123")
...
class TestContractBusinessRules:
    ...
    def test_cannot_activate_expired_contract(self):
        ...
        # When & Then
        with pytest.raises(BusinessRuleViolationError, match="Cannot activate expired contract"):
            contract.activate("user-123")
    ...
    def test_cannot_modify_completed_contract(self):
        ...
        # When & Then
        with pytest.raises(BusinessRuleViolationError, match="Cannot modify completed contract"):
    ...
    def test_compliance_score_must_be_between_zero_and_one(self):
        ...
        # When & Then
        with pytest.raises(ValueError, match="Compliance score must be between 0 and 1"):
