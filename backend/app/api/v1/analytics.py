...
from app.infrastructure.database.models import (
    ...
    AuditLog, SystemMetrics, ContractStatus, ContractType
)
...
async def get_business_metrics(
    ...
    # Compliance metrics
    compliance_avg = db.query(
        func.avg(ComplianceScore.overall_score)
    ).join(Contract).filter(
        Contract.company_id == company.id
    ).scalar() or 0.8
    
    high_risk_contracts = db.query(ComplianceScore).join(Contract).filter(
        Contract.company_id == company.id,
    ...
    contracts_last_month = db.query(Contract).filter(
        Contract.company_id == company.id,
    ...
    if contracts_last_month > 0:
        growth_rate = ((contracts_this_month - contracts_last_month) / contracts_last_month) * 100
...
async def get_user_metrics(
    ...
    this_month_start = get_current_utc().replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    ...
    # User engagement
    user_engagement_score = (active_users_30d / total_users * 100) if total_users > 0 else 0
...
async def get_contract_type_metrics(
    ...
    for stat in type_stats:
        ...
        results.append(ContractTypeMetrics(
            contract_type=stat.contract_type.value,
            count=stat.count,
            percentage=(stat.count / total_contracts * 100) if total_contracts > 0 else 0,
...
async def get_time_series_metrics(
    ...
    if len(data_points) >= 4:
        ...
        second_half_avg = sum(dp.value for dp in data_points[mid_point:]) / (len(data_points) - mid_point)
...
async def get_compliance_metrics(
    ...
    # Risk distribution
    high_risk = db.query(ComplianceScore).join(Contract).filter(
        Contract.company_id == company.id,
        ComplianceScore.risk_score >= 7
    ).count()
    
    medium_risk = db.query(ComplianceScore).join(Contract).filter(
        Contract.company_id == company.id,
        ComplianceScore.risk_score >= 4,
        ComplianceScore.risk_score < 7
    ).count()
    
    low_risk = db.query(ComplianceScore).join(Contract).filter(
        Contract.company_id == company.id,
        ComplianceScore.risk_score < 4
    ).count()
    ...
    recommendations_count = db.query(
        func.sum(func.json_array_length(ComplianceScore.recommendations))
    ).join(Contract).filter(
        Contract.company_id == company.id
    ).scalar() or 0
    ...
    return ComplianceMetricsResponse(
        ...
        gdpr_compliance_average=float(compliance_stats.gdpr) if compliance_stats.gdpr else 0.8,
        employment_law_compliance_average=float(compliance_stats.employment) if compliance_stats.employment else 0.8,
        consumer_rights_compliance_average=float(compliance_stats.consumer) if compliance_stats.consumer else 0.8,
        commercial_terms_compliance_average=float(compliance_stats.commercial) if compliance_stats.commercial else 0.8,
...
async def get_performance_metrics(
    ...
    ai_stats = db.query(
        ...
        func.sum(
            func.json_extract(AIGeneration.token_usage, '$.total_tokens').cast(db.Integer)
        ).label('total_tokens'),
        func.avg(
            func.json_extract(AIGeneration.token_usage, '$.total_tokens').cast(db.Integer)
        ).label('avg_tokens'),
        ...
        AIGeneration.created_at >= get_current_utc() - timedelta(days=7)
    ).first()
