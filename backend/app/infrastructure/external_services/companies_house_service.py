...
class CompaniesHouseService:
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    company_url, auth=aiohttp.BasicAuth(self.api_key, "")
                ) as response:
                ...
                try:
                    ...
                    async with session.get(
                        officers_url, auth=aiohttp.BasicAuth(self.api_key, "")
                        ...
                        if officers_response.status == 200:
                            ...
                            officers = self._parse_officers(
                                officers_data.get("items", [])
                            )
                except Exception as e:
                    self.logger.warning(
                        f"Failed to fetch officers for {clean_number}: {e}"
                    )
    ...
    async def search_companies(
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    search_url, params=params, auth=aiohttp.BasicAuth(self.api_key, "")
                ) as response:
    ...
    async def get_company_filing_history(
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                async with session.get(
                    filing_url, params=params, auth=aiohttp.BasicAuth(self.api_key, "")
                ) as response:
    ...
    def _parse_company_data(
        self, data: Dict[str, Any], officers: List[CompanyHouseOfficer]
    ) -> CompanyHouseData:
    ...
    def _parse_officers(
        self, officers_data: List[Dict[str, Any]]
    ) -> List[CompanyHouseOfficer]:
    ...
    def format_company_verification_data(
        self, ch_data: CompanyHouseData
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "date_of_creation": (
                ch_data.date_of_creation.isoformat()
                if ch_data.date_of_creation
                else None
            ),
            "registered_office_address": (
                ch_data.registered_office_address.to_uk_address()
                if ch_data.registered_office_address
                else None
            ),
