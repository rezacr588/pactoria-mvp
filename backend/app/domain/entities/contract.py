...
from app.domain.value_objects import (
    ContractStatus, ContractType, Money, DateRange, Email, 
    ContractParty, ComplianceScore, RiskAssessment, ContractVersion
)
...
class Contract(AggregateRoot[ContractId]):
    ...
    def set_compliance_analysis(self, compliance_score: ComplianceScore, risk_assessment: RiskAssessment):
    ...
    def activate(self, user_id: str):
        ...
        if not self._final_content:
            raise BusinessRuleViolationError("Cannot activate contract without final content")
        ...
        event = ContractActivated.create(
            ...
            contract_value=self._contract_value.amount if self._contract_value else None,
    ...
    def complete(self, user_id: str, completion_reason: Optional[str] = None):
        ...
        if self._status != ContractStatus.ACTIVE:
            raise BusinessRuleViolationError("Cannot complete contract that is not active")
    ...
    def _validate_title(self, title: str):
        ...
        if not title or not title.strip():
            raise DomainValidationError("Contract title cannot be empty", "title", title)
        
        if len(title.strip()) > 200:
            raise DomainValidationError("Contract title too long (max 200 characters)", "title", title)
    
    def _validate_plain_english_input(self, input_text: str):
        """Validate plain English input"""
        if not input_text or not input_text.strip():
            raise DomainValidationError("Plain English input cannot be empty", "plain_english_input", input_text)
        
        if len(input_text.strip()) < 10:
            raise DomainValidationError("Plain English input too short (min 10 characters)", "plain_english_input", input_text)
