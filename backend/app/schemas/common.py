...
class ErrorDetail(BaseModel):
    ...
    loc: List[str] = Field(..., description="Error location path", example=["body", "email"])
...
class ErrorResponse(BaseModel):
    ...
    error_code: Optional[str] = Field(None, description="Application-specific error code", example="RESOURCE_NOT_FOUND")
    path: Optional[str] = Field(None, description="Request path", example="/api/v1/contracts/invalid-id")
    method: Optional[str] = Field(None, description="HTTP method", example="GET")
    timestamp: Optional[datetime] = Field(None, description="Error timestamp", example="2024-08-31T14:20:30Z")
...
class PaginationParams(BaseModel):
    ...
    size: int = Field(20, ge=1, le=100, description="Items per page (max 100)", example=20)
...
class HealthResponse(BaseModel):
    ...
    timestamp: float = Field(..., description="Response timestamp", example=1693564830.123)
