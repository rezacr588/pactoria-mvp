...
from app.domain.entities.team import Team, TeamMember, TeamInvitation, UserRole
from app.domain.value_objects import Email
from app.domain.exceptions import DomainValidationError, ResourceNotFoundError
...
class TeamApplicationService:
    ...
    async def invite_team_member(
        self, command: InviteTeamMemberCommand
    ) -> InvitationDto:
    ...
    async def remove_team_member(
        self, command: RemoveTeamMemberCommand
    ) -> Dict[str, Any]:
        ...
        team.remove_member(
            member_id=command.member_id, removed_by_user_id=command.removed_by_user_id
        )
    ...
    async def resend_invitation(
        self, command: ResendInvitationCommand
    ) -> Dict[str, Any]:
        ...
        invitation = next(
            (
                inv
                for inv in team.pending_invitations
                if inv.id.value == command.invitation_id
            ),
            None,
        )
    ...
    async def get_team_members(
        ...
        if include_inactive:
            ...
            if department_filter:
                invitations = [
                    inv for inv in invitations if inv.department == department_filter
                ]
    ...
    async def get_team_member(self, company_id: str, member_id: str) -> TeamMemberDto:
        ...
        member = next(
            (member for member in team.members if member.id.value == member_id), None
        )
    ...
    async def get_team_statistics(self, company_id: str) -> TeamStatsDto:
        ...
        recent_members = sorted(
            team.active_members, key=lambda m: m.created_at, reverse=True
        )[:5]
        ...
        recent_invitations = sorted(
            team.pending_invitations, key=lambda i: i.created_at, reverse=True
        )[:5]
    ...
    def _map_invitation_to_member_dto(
