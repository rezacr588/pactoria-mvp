...
class TestEdgeCasesAndErrorScenarios:
                ...
                for user in users:
                    for contract_index in range(2):  # 2 contracts per user
                        future = executor.submit(
                            generate_contract_for_user, user, contract_index
                        )
        ...
        extreme_inputs = [
            ...
            },
            {
                ...
                "data": {
                    ...
                    "contract_type": "service_agreement",
                    "plain_english_input": "This is a test. "
                    * 1000,  # Very long input (50k chars)
                },
            },
            {
                "name": "empty_strings",
    ...
    def test_ai_service_failures_and_fallbacks(self, client):
        ...
        failure_scenarios = [
            ...
            {"name": "rate_limit_error", "exception": Exception("Rate limit exceeded")},
            {"name": "invalid_api_key", "exception": Exception("Invalid API key")},
            {
    ...
    def test_database_constraint_violations(self, client):
        ...
        response = client.get(
            "/api/v1/contracts/non-existent-contract-id", headers=auth_headers
        )
    ...
    def test_subscription_and_rate_limits(self, client):
        ...
        free_user_data = {
            ...
            "company": {"name": "Free Tier Company", "registration_number": "99999999"},
        }
        ...
        for sequence in failure_sequences:
            for request_spec in sequence:
                if request_spec["method"] == "GET":
                    response = client.get(
                        request_spec["endpoint"], headers=auth_headers
                    )
                else:
                    response = client.post(
