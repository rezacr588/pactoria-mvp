...
class TestAdvancedSearchFlows:
    ...
    def test_comprehensive_contract_search_workflow(
        ...
        with performance_helper.measure_time("basic_search"):
            search_response = client.get(
                "/api/v1/contracts/?search=Technology", headers=headers
            )
        ...
        tech_contracts = [
            c
            for c in search_results["contracts"]
            if "Tech" in c["title"] or "Tech" in c["client_name"]
        ]
        assert len(tech_contracts) >= 1

        # Step 3: Test contract type filtering
        service_agreements_response = client.get(
            "/api/v1/contracts/?contract_type=service_agreement", headers=headers
        )
        ...
        active_contracts_response = client.get(
            "/api/v1/contracts/?status=active", headers=headers
        )
        ...
        for contract in combined_results["contracts"]:
            ...
            assert contract["status"] == "active"
            assert (
                "Professional" in contract["title"]
                or "Professional" in contract["supplier_name"]
            )

        # Step 6: Test pagination
        paginated_response = client.get(
            "/api/v1/contracts/?page=1&size=3", headers=headers
        )
        ...
        performance_helper.assert_performance(
            "contract_creation", 2000
        )  # 2 seconds per creation
    ...
    def test_advanced_search_with_complex_filters(self, e2e_test_base: E2ETestBase):
        ...
        advanced_search_response = client.post(
        ...
        for contract in quick_results.get(
            "contracts", quick_results.get("results", [])
        ):
            if "contract_type" in contract:
                assert contract["contract_type"] in [
                    "service_agreement",
                    "SERVICE_AGREEMENT",
                ]
        ...
        suggestions_response = client.get(
            "/api/v1/search/suggestions/contracts?q=serv&limit=5", headers=headers
        )
    ...
    def test_user_and_template_search(self, e2e_test_base: E2ETestBase):
        ...
        legal_user = e2e_test_base.create_test_user(
            company,
            {
        ...
        user_search_data = {
            "query": "legal",
            "filters": {"role": ["legal_reviewer"], "department": "Legal"},
            "page": 1,
            "size": 10,
        }

        user_search_response = client.post(
            "/api/v1/search/users", json=user_search_data, headers=admin_headers
        )
        ...
        legal_users = [
            ...
            if "Legal" in u.get("full_name", "") or u.get("department") == "Legal"
        ]
        ...
        template_search_response = client.post(
            "/api/v1/search/templates", json=template_search_data, headers=admin_headers
        )
        ...
        found_templates = template_results.get(
            "templates", template_results.get("results", [])
        )
        professional_templates = [
            t for t in found_templates if "Professional" in t.get("name", "")
        ]
...
class TestAnalyticsWorkflows:
    ...
    def test_complete_dashboard_analytics_workflow(
        self, e2e_test_base: E2ETestBase, performance_helper
    ):
        ...
        contract_types_response = client.get(
        ...
        assert (
            len(contract_types) >= 4
        )  # service_agreement, employment_contract, nda, consultancy

        # Verify service agreements are most common
        service_agreements = [
            ct for ct in contract_types if ct["contract_type"] == "service_agreement"
        ]
        ...
        compliance_response = client.get(
            "/api/v1/analytics/compliance", headers=headers
        )
    ...
    def test_admin_only_analytics(self, e2e_test_base: E2ETestBase):
        ...
        system_health_response = client.get(
            "/api/v1/analytics/system/health", headers=admin_headers
        )
        ...
        performance_response = client.get(
            "/api/v1/analytics/performance", headers=admin_headers
        )
        ...
        regular_health_response = client.get(
            "/api/v1/analytics/system/health", headers=regular_headers
        )
        e2e_test_base.assert_response_error(regular_health_response, 403)

        regular_performance_response = client.get(
            "/api/v1/analytics/performance", headers=regular_headers
        )
        e2e_test_base.assert_response_error(regular_performance_response, 403)

    def test_analytics_with_large_dataset(
        self, e2e_test_base: E2ETestBase, performance_helper
    ):
