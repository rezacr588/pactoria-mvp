...
from app.domain.value_objects import (
    ContractStatus, ContractType, Email, ContractParty, Money, ComplianceScore, RiskAssessment
)
...
from app.application.services.contract_application_service import (
    ContractApplicationService, CreateContractCommand, UpdateContractCommand,
    GenerateContractContentCommand, AnalyzeComplianceCommand, ActivateContractCommand
)
...
class TestContractApplicationService:
    ...
    @pytest.fixture
    def application_service(self, mock_contract_repository, mock_ai_service, mock_event_publisher) -> ContractApplicationService:
    ...
    async def test_update_nonexistent_contract_raises_not_found_error(
        ...
        mock_contract_repository.get_by_id = AsyncMock(side_effect=ContractNotFoundError("non-existent"))
    ...
    async def test_analyze_compliance_analyzes_and_saves_results(
        ...
        mock_contract_repository.get_by_id = AsyncMock(return_value=sample_contract)
        mock_ai_service.validate_contract_compliance = AsyncMock(return_value=compliance_result)
        mock_contract_repository.save = AsyncMock(return_value=sample_contract)
    ...
    async def test_get_contracts_by_company_returns_paginated_results(
        ...
        # When
        result = await application_service.get_contracts_by_company(company_id, page, size)
