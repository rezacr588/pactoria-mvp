...
from app.infrastructure.database.models import (
    User, Company, Contract, Template, 
    ContractStatus, ContractType, SubscriptionTier, UserRole
)
...
class TestDataFactory:
    ...
    @staticmethod
    def create_user_data(company_id: str = None, role: UserRole = UserRole.CONTRACT_MANAGER) -> dict:
...
class TestAuthenticationEndpoints:
    ...
    def test_user_login_flow(self, test_client, test_db_manager):
        ...
        try:
            ...
            response = test_client.post("/api/v1/auth/login", data=login_data, headers={"Content-Type": "application/x-www-form-urlencoded"})
            assert response.status_code == 200
            data = response.json()
    ...
    def test_create_contract(self, test_client, test_db_manager):
        ...
        try:
            ...
            with patch('app.services.ai_service.ai_service') as mock_ai:
                mock_ai.generate_contract = AsyncMock(return_value=Mock(
                    content="MOCK_CONTRACT_CONTENT",
                    model_name="test-model",
                    processing_time_ms=1000
                ))
                
                response = test_client.post("/api/v1/contracts/", json=contract_data, headers=headers)
                assert response.status_code == 201
                data = response.json()
    ...
    def test_get_contract_by_id(self, test_client, test_db_manager):
        ...
        try:
            ...
            with patch('app.services.ai_service.ai_service') as mock_ai:
                ...
                create_response = test_client.post("/api/v1/contracts/", json=contract_data, headers=headers)
                assert create_response.status_code == 201
                contract_id = create_response.json()["id"]
                
                # Get contract
                response = test_client.get(f"/api/v1/contracts/{contract_id}", headers=headers)
                assert response.status_code == 200
                data = response.json()
...
class TestAIServiceEndpoints:
    ...
    def test_ai_health_check(self, test_client):
        ...
        with patch('app.services.ai_service.ai_service') as mock_ai:
            mock_ai.health_check = AsyncMock(return_value={
                "status": "healthy",
                "model": "test-model",
                "response_time_ms": 45
            })
    ...
    def test_ai_contract_analysis(self, test_client, test_db_manager):
        ...
        try:
            ...
            with patch('app.services.ai_service.ai_service') as mock_ai:
                mock_ai.analyze_compliance = AsyncMock(return_value=Mock(
                    overall_score=0.95,
                    risk_score=2,
                ...
                response = test_client.post("/api/v1/ai/analyze-contract", json=analysis_request, headers=headers)
                assert response.status_code == 200
                data = response.json()
...
class TestErrorHandling:
    ...
    def test_invalid_contract_creation(self, test_client, test_db_manager):
        ...
        try:
            ...
            response = test_client.post("/api/v1/contracts/", json=invalid_data, headers=headers)
