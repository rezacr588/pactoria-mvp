...
class TestAIModelAccuracy:
    ...
    @pytest.mark.asyncio
    async def test_contract_generation_accuracy_service_agreement(
        self, ai_service, sample_contracts
    ):
...
"""
        ...
        assert (
            accuracy_score >= 0.80
        ), f"Accuracy {accuracy_score:.2%} below threshold. Missing: {missing_elements}"
        ...
        found_uk_indicators = sum(
            1 for indicator in uk_indicators if indicator in content_lower
        )
        assert found_uk_indicators >= 1, "Contract should indicate UK jurisdiction"

    @pytest.mark.asyncio
    async def test_contract_generation_accuracy_employment_contract(
        self, ai_service, sample_contracts
    ):
        ...
        assert (
            accuracy_score >= 0.75
        ), f"Employment contract accuracy {accuracy_score:.2%} below threshold"
        ...
        found_employment = sum(
            1 for req in employment_requirements if req in content_lower
        )
        ...
        found_gdpr = sum(
            1 for indicator in gdpr_indicators if indicator in content_lower
        )
        assert (
            found_gdpr >= 1
        ), "Employment contract should include GDPR/data protection clauses"
    ...
    async def test_compliance_analysis_accuracy(self, ai_service):
        ...
        assert (
            response.overall_score < 0.8
        ), "Poor contract should have low compliance score"
        ...
        assert any(
            term in risk_factors_text for term in ["payment", "vague", "unclear"]
        ), "Should identify vague payment terms as risk"

        # Should recommend GDPR improvements
        assert any(
            term in recommendations_text
    ...
    async def test_high_quality_contract_compliance_analysis(self, ai_service):
        ...
        ), f"High-quality contract scored {response.overall_score:.2%}"
        assert (
            response.gdpr_compliance >= 0.90
        ), f"GDPR compliance scored {response.gdpr_compliance:.2%}"
        assert response.risk_score <= 4, f"Risk score too high: {response.risk_score}"

        # Should have minimal risk factors
        assert (
            len(response.risk_factors) <= 3
        ), "High-quality contract should have few risk factors"
    ...
    async def test_ai_model_consistency(self, ai_service):
        ...
        for response_content in responses:
            ...
            ), f"Consistency score {consistency_score:.2%} too low"
    ...
    async def test_ai_response_time_performance(self, ai_service):
        ...
        requests = [
            ...
            AIGenerationRequest(
                prompt="Medium contract section with terms", max_tokens=500
            ),
            AIGenerationRequest(
                prompt="Full contract generation with multiple sections",
                max_tokens=1500,
            ),
        ]
        ...
        for i, request in enumerate(requests):
            ...
            assert (
                actual_time < 30000
            ), f"Request {i+1} took {actual_time:.0f}ms (over 30s limit)"

            # Verify reported time is reasonable
            assert (
                abs(actual_time - response.processing_time_ms) < 1000
    ...
    async def test_contract_type_specialization(self, ai_service, sample_contracts):
        ...
        for contract_type in contract_types:
            ...
            request = ContractGenerationRequest(
                ...
                contract_value=(
                    50000.0 if contract_type != "employment_contract" else 75000.0
                ),
            )
    ...
    async def test_gdpr_compliance_accuracy(self, ai_service):
        ...
        compliant_request = ComplianceAnalysisRequest(
            contract_content=gdpr_compliant_contract, contract_type="service_agreement"
        )
        ...
        non_compliant_request = ComplianceAnalysisRequest(
            contract_content=non_compliant_contract, contract_type="service_agreement"
        )
        non_compliant_response = await ai_service.analyze_compliance(
            non_compliant_request
        )

        # GDPR-compliant contract should score higher
        assert (
            compliant_response.gdpr_compliance > non_compliant_response.gdpr_compliance
        ), "GDPR-compliant contract should score higher than non-compliant one"

        assert (
            compliant_response.gdpr_compliance >= 0.90
        ), f"GDPR-compliant contract scored only {compliant_response.gdpr_compliance:.2%}"

        assert (
            non_compliant_response.gdpr_compliance <= 0.70
    ...
    async def test_ai_model_token_usage_efficiency(self, ai_service):
        ...
        test_cases = [
            ...
            (
                "Medium request",
                "Generate a full confidentiality section with multiple clauses",
                500,
            ),
            (
                "Long request",
                "Generate a complete service agreement with all standard clauses",
                1500,
            ),
        ]
        ...
        for description, prompt, max_tokens in test_cases:
            request = AIGenerationRequest(
                prompt=prompt, max_tokens=max_tokens, temperature=0.5
            )
    ...
    async def test_confidence_score_accuracy(self, ai_service):
        ...
        test_requests = [
            (
                "High quality prompt",
                "Generate a professional employment contract with standard UK terms including salary, benefits, and termination clauses.",
            ),
            ("Vague prompt", "Make a contract."),
            (
                "Detailed prompt",
                "Create a comprehensive service agreement for IT consulting services including payment terms, deliverables, IP ownership, confidentiality, and UK legal compliance.",
            ),
        ]
        ...
        for description, prompt in test_requests:
            ...
            confidence_scores.append(
                (description, response.confidence_score, len(response.content))
            )

        # More detailed prompts should generally have higher confidence scores
        detailed_score = next(
            score for desc, score, _ in confidence_scores if "Detailed" in desc
        )
        vague_score = next(
            score for desc, score, _ in confidence_scores if "Vague" in desc
        )

        assert (
            detailed_score >= vague_score
    ...
    async def test_health_check_functionality(self, ai_service):
        ...
        assert (
            health_check_time < 10000
        ), f"Health check took {health_check_time:.0f}ms (should be under 10s)"
    ...
    async def test_error_handling_robustness(self, ai_service):
        ...
        invalid_requests = [
            ...
            AIGenerationRequest(
                prompt="x" * 10000, max_tokens=1
            ),  # Very long prompt, tiny response
        ]
    ...
    async def test_uk_legal_terminology_accuracy(self, ai_service):
        ...
        assert (
            found_structural >= 3
        ), f"Contract should use proper legal structure (found {found_structural} terms)"
        ...
        assert (
            found_us_terms == 0
        ), f"Contract should not use US legal terminology (found: {found_us_terms})"
