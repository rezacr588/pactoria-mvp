...
class BaseAIService(ABC):
    ...
    @abstractmethod
    async def generate_contract(self, request: ContractGenerationRequest) -> AIGenerationResult:
        """Generate contract content using AI"""
        pass
    
    @abstractmethod
    async def analyze_contract_compliance(self, contract_content: str) -> Dict[str, Any]:
...
class OpenAIContractService(BaseAIService):
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "gpt-4", max_retries: int = 3):
    ...
    async def analyze_contract_compliance(self, contract_content: str) -> Dict[str, Any]:
        ...
        user_prompt = f"Analyze this contract for UK legal compliance:\n\n{contract_content}"
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    continue
                raise AIServiceError("OpenAI rate limit exceeded", "openai", "rate_limit")
    ...
    def _parse_openai_response(self, response: Any, processing_time: float) -> AIGenerationResult:
    ...
    def _analyze_content_for_warnings(self, content: str) -> List[str]:
        ...
        if len(content) < 1000:
            warnings.append("Generated content may be too short for a complete contract")
    ...
    def _generate_content_suggestions(self, content: str) -> List[str]:
        ...
        if "shall" not in content.lower():
            suggestions.append("Consider using 'shall' for obligations to improve legal clarity")
...
class AnthropicContractService(BaseAIService):
    ...
    def __init__(self, api_key: Optional[str] = None, model: str = "claude-3-sonnet-20240229", max_retries: int = 3):
    ...
    def _parse_anthropic_response(self, response: Any, processing_time: float) -> AIGenerationResult:
...
class AIContractService:
    ...
    async def generate_contract(self, request: ContractGenerationRequest) -> AIGenerationResult:
        ...
        if self.primary_provider in self.services:
            try:
                return await self.services[self.primary_provider].generate_contract(request)
            except AIServiceError as e:
                self.logger.warning(f"Primary provider {self.primary_provider} failed: {e}")
