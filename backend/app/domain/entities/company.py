...
class Company(AggregateRoot[CompanyId]):
    ...
    def __init__(
        ...
        self._compliance_requirements: List[str] = (
            self._determine_compliance_requirements()
        )
    ...
    def from_persistence(
        ...
        company._features_enabled = features_enabled or [
            "contract_generation",
            "team_management",
        ]
    ...
    def verify_with_companies_house(
        self, verification_data: Dict[str, Any], user_id: str
    ):
        """Mark company as verified via Companies House"""
        if not self._company_number:
            raise BusinessRuleViolationError(
                "Cannot verify company without company number"
            )
