...
class TestTeamManagement:
    ...
    def test_get_team_members_success(self):
        ...
        regular_user = create_test_user(
            self.db, company.id, UserRole.CONTRACT_MANAGER, email="user@test.com"
        )
    ...
    def test_invite_team_member_success(self):
        ...
        invitation = (
            ...
            .filter(TeamInvitation.email == invitation_data["email"])
            .first()
        )
    ...
    def test_get_team_member_by_id_success(self):
        ...
        target_user = create_test_user(
            self.db, company.id, UserRole.CONTRACT_MANAGER, email="target@test.com"
        )
...
class TestTeamManagementBusinessLogic:
    ...
    def test_invitation_expiry_logic(self):
        ...
        valid_invitation = TeamInvitation(expires_at=now + timedelta(days=1))
        assert valid_invitation.expires_at > now

        # Expired invitation
        expired_invitation = TeamInvitation(expires_at=now - timedelta(days=1))
