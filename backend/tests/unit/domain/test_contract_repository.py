...
class TestContractRepository:
    ...
    @pytest.mark.asyncio
    async def test_save_contract_returns_contract(
        self, mock_repository: Mock, sample_contract: Contract
    ):
    ...
    @pytest.mark.asyncio
    async def test_find_by_company_returns_contracts(
        self, mock_repository: Mock, sample_contract: Contract
    ):
    ...
    @pytest.mark.asyncio
    async def test_find_with_filter_returns_filtered_contracts(
        self, mock_repository: Mock, sample_contract: Contract
    ):
        """Test finding contracts with filter"""
        # Given
        contract_filter = ContractFilter(
            contract_type=ContractType.SERVICE_AGREEMENT, status=ContractStatus.DRAFT
        )
        expected_contracts = [sample_contract]
...
class TestContractFilter:
    ...
    def test_filter_equality(self):
        ...
        filter1 = ContractFilter(
            company_id="company-456", contract_type=ContractType.SERVICE_AGREEMENT
        )
        filter2 = ContractFilter(
            company_id="company-456", contract_type=ContractType.SERVICE_AGREEMENT
        )
        filter3 = ContractFilter(
            company_id="company-789", contract_type=ContractType.SERVICE_AGREEMENT
        )
