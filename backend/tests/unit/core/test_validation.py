...
class TestResourceValidator:
    ...
    @patch("app.core.validation.require_company_access")
    def test_validate_contract_exists_and_access_no_permission(
        self, mock_require_access
    ):
        ...
        mock_require_access.side_effect = HTTPException(
    ...
    @patch("app.core.validation.require_company_access")
    def test_validate_company_exists_and_access_no_permission(
        self, mock_require_access
    ):
...
class TestAuditLogHelper:
    ...
    def test_create_audit_log_batch(self):
        ...
        AuditLogHelper.log_batch_actions(db=mock_db, user=mock_user, actions=actions)
...
class TestValidationHelpers:
    ...
    def test_validate_enum_value_valid(self):
        ...
        result = ResourceValidator.validate_enum_value(
            "service_agreement", ContractType, "contract_type"
        )
    ...
    def test_validate_enum_value_invalid(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            ResourceValidator.validate_enum_value(
                "invalid_type", ContractType, "contract_type"
            )
