...
class TestContractLifecycleFlows:
    ...
    def test_complete_contract_creation_workflow(
        ...
        with performance_helper.measure_time("compliance_analysis"):
            analysis_data = {"force_reanalysis": False}
        ...
        final_content = (
            updated_contract["generated_content"]
        ...
        complete_response = client.put(
            f"/api/v1/contracts/{contract_id}", json=completion_data, headers=headers
        )
    ...
    def test_template_based_contract_creation(self, e2e_test_base: E2ETestBase):
        ...
        contract_response = client.get(
            f"/api/v1/contracts/{created_contract['id']}", headers=headers
        )
        e2e_test_base.assert_response_success(contract_response)
    ...
    def test_contract_collaboration_workflow(self, e2e_test_base: E2ETestBase):
        ...
        company = e2e_test_base.create_test_company()
        creator_user = e2e_test_base.create_test_user(
            company, {"full_name": "Contract Creator"}
        )
        reviewer_user = e2e_test_base.create_test_user(
            company, {"full_name": "Legal Reviewer"}
        )
        approver_user = e2e_test_base.create_test_user(
            company, {"full_name": "Contract Approver"}
        )
        ...
        create_response = client.post(
            "/api/v1/contracts/", json=contract_data, headers=creator_headers
        )
        e2e_test_base.assert_response_success(create_response, 201)
        ...
        review_response = client.get(
            f"/api/v1/contracts/{contract_id}", headers=reviewer_headers
        )
        ...
        final_review_response = client.get(
            f"/api/v1/contracts/{contract_id}", headers=approver_headers
        )
        ...
        for user_headers in [creator_headers, reviewer_headers, approver_headers]:
            access_response = client.get(
                f"/api/v1/contracts/{contract_id}", headers=user_headers
            )
    ...
    def test_contract_regeneration_workflow(self, e2e_test_base: E2ETestBase):
        ...
        contract_response_1 = client.get(
            f"/api/v1/contracts/{contract_id}", headers=headers
        )
        ...
        contract_response_2 = client.get(
            f"/api/v1/contracts/{contract_id}", headers=headers
        )
        ...
        assert (
            third_generation["generated_content"] == second_content
        )  # Should be cached
...
class TestContractStateTransitions:
    ...
    def test_contract_termination_workflow(self, e2e_test_base: E2ETestBase):
        ...
        # Draft contract
        draft_contract = e2e_test_base.create_test_contract(
            company,
            user,
            {"title": "Draft Contract for Termination", "status": ContractStatus.DRAFT},
        )

        # Active contract
        active_contract = e2e_test_base.create_test_contract(
            company,
            user,
            {
        ...
        active_contract_ids = [
            c["id"] for c in contracts if c["status"] != "terminated"
        ]
...
class TestContractValidationRules:
    ...
    def test_contract_creation_validation(
    ...
    def test_contract_business_rules(self, e2e_test_base: E2ETestBase):
        ...
        high_value_response = client.post(
            "/api/v1/contracts/", json=high_value_contract, headers=headers
        )
        ...
        invalid_date_response = client.post(
            "/api/v1/contracts/", json=invalid_date_contract, headers=headers
        )
