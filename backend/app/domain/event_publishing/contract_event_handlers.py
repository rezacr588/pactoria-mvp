...
class ContractCreatedEventHandler(AuditEventHandler):
    ...
    async def _handle_event(self, event: DomainEvent) -> None:
        ...
        self._logger.info(
            f"Processed ContractCreated event for contract {event.aggregate_id}"
        )
...
class ContractActivatedEventHandler(NotificationEventHandler, AuditEventHandler):
    ...
    async def _handle_event(self, event: DomainEvent) -> None:
        ...
        recipients = [
            event.activated_by_user_id
        ]  # Could expand to include other stakeholders
        await self._send_notification(message, recipients, "contract_activation")

        self._logger.info(
            f"Processed ContractActivated event for contract {event.aggregate_id}"
        )
...
class ContractCompletedEventHandler(NotificationEventHandler, AuditEventHandler):
    ...
    async def _handle_event(self, event: DomainEvent) -> None:
        ...
        self._logger.info(
            f"Processed ContractCompleted event for contract {event.aggregate_id}"
        )
    ...
    async def _handle_event(self, event: DomainEvent) -> None:
        ...
        if not isinstance(event, ContractTerminated):
            self._logger.warning(
                f"Expected ContractTerminated event, got {type(event)}"
            )
            return
        ...
        self._logger.warning(
            f"Contract {event.aggregate_id} terminated: {event.termination_reason}"
        )
...
class ContractContentGeneratedEventHandler(AuditEventHandler):
    ...
    async def _handle_event(self, event: DomainEvent) -> None:
        ...
        if not isinstance(event, ContractContentGenerated):
            self._logger.warning(
                f"Expected ContractContentGenerated event, got {type(event)}"
            )
            return
