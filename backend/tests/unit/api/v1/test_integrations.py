...
class TestConnectIntegration:
    ...
    def test_connect_integration_success(self, mock_auth):
        ...
        integration_id = "1"
        connection_data = {
            "configuration": {"api_key": "test_api_key", "account_id": "test_account"},
            "api_key": "test_api_key",
        }

        response = client.post(
            f"/api/v1/integrations/{integration_id}/connect", json=connection_data
        )
    ...
    def test_connect_integration_empty_body(self, mock_auth):
        ...
        response = client.post(
            f"/api/v1/integrations/{integration_id}/connect", json={}
        )
...
class TestConfigureIntegration:
    ...
    def test_configure_integration_success(self, mock_auth):
        ...
        response = client.put(
            f"/api/v1/integrations/{integration_id}/configure", json=config_data
        )
...
class TestIntegrationEndpointsIntegration:
    ...
    def test_integration_management_workflow(self, mock_auth):
        ...
        if integrations:
            ...
            # 7. Get sync status
            sync_status_response = client.get(
                f"/api/v1/integrations/{integration_id}/sync-status"
            )
...
class TestIntegrationValidation:
    ...
    def test_connect_integration_validation(self, mock_auth):
        ...
        valid_data = {"configuration": {"api_key": "test_key"}}
        response = client.post(
            f"/api/v1/integrations/{integration_id}/connect", json=valid_data
        )
        assert response.status_code == 200
    ...
    def test_configure_integration_validation(self, mock_auth):
        ...
        # Test with valid data
        valid_data = {"configuration": {"setting1": "value1", "setting2": "value2"}}
        response = client.put(
            f"/api/v1/integrations/{integration_id}/configure", json=valid_data
        )
        assert response.status_code == 200

        # Test missing configuration should fail
        response = client.put(
            f"/api/v1/integrations/{integration_id}/configure", json={}
        )
    ...
    def test_invalid_integration_id(self, mock_auth):
        ...
        response = client.post(
            f"/api/v1/integrations/{invalid_id}/connect", json={"configuration": {}}
        )
        assert response.status_code == 200
