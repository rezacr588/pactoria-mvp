...
from app.domain.value_objects import (
    ContractStatus, ContractType, Email, ContractParty, Money, 
    ComplianceScore, RiskAssessment, DateRange
)
from app.domain.repositories.contract_repository import (
    ContractRepository, ContractFilter, PageRequest, PageResult
)
from app.domain.exceptions import BusinessRuleViolationError, ContractNotFoundError
...
class ContractApplicationService:
    ...
    def __init__(self, 
                 ...
                 event_publisher: Optional[Any] = None):  # Domain event publisher
    ...
    async def update_contract(self, command: UpdateContractCommand) -> Contract:
        ...
        if command.contract_value is not None:
            contract.set_contract_value(Money(command.contract_value, command.currency or "GBP"))
        
        if command.final_content is not None:
            contract.finalize_content(command.final_content, command.updated_by_user_id or "system")
        
        if command.start_date is not None or command.end_date is not None:
            # Use existing dates if not provided
            start_date = command.start_date or (contract.date_range.start_date if contract.date_range else None)
            end_date = command.end_date or (contract.date_range.end_date if contract.date_range else None)
    ...
    async def generate_contract_content(self, command: GenerateContractContentCommand) -> Contract:
        ...
        ai_request = ContractGenerationRequest(
            ...
            contract_value=float(contract.contract_value.amount) if contract.contract_value else None,
            currency=contract.contract_value.currency if contract.contract_value else "GBP",
            start_date=contract.date_range.start_date.isoformat() if contract.date_range and contract.date_range.start_date else None,
            end_date=contract.date_range.end_date.isoformat() if contract.date_range and contract.date_range.end_date else None
        )
    ...
    async def analyze_compliance(self, command: AnalyzeComplianceCommand) -> Contract:
        ...
        if not content:
            raise BusinessRuleViolationError("No content to analyze. Generate or add content first.")
        ...
        compliance_score = ComplianceScore(
            ...
            employment_law_compliance=compliance_result.get("employment_law_compliance"),
            consumer_rights_compliance=compliance_result.get("consumer_rights_compliance"),
            commercial_terms_compliance=compliance_result.get("commercial_terms_compliance")
        )
    ...
    async def get_contracts_by_company(self, company_id: str, page: int = 1, size: int = 20) -> PageResult:
    ...
    async def search_contracts(self, filters: Dict[str, Any], page: int = 1, size: int = 20) -> PageResult:
        """Search contracts with filters"""
        # Convert filters dict to ContractFilter
        contract_filter = self._create_contract_filter(filters)
        page_request = PageRequest(page=page, size=size)
        
        return await self._contract_repository.find_with_filter(contract_filter, page_request)
    
    async def get_expiring_contracts(self, company_id: str, days_ahead: int = 30) -> List[Contract]:
    ...
    async def get_contracts_requiring_review(self, company_id: str) -> List[Contract]:
        ...
        return await self._contract_repository.find_contracts_requiring_compliance_review(company_id)
    ...
    def _create_contract_filter(self, filters: Dict[str, Any]) -> ContractFilter:
        ...
        return ContractFilter(
            ...
            contract_type=ContractType(filters["contract_type"]) if filters.get("contract_type") else None,
