...
class TestComplianceAndRecovery:
    ...
    def test_gdpr_data_portability_requirement(self, client, auth_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            response = client.get(
                "/api/v1/compliance/gdpr/data-export",
    ...
    def test_gdpr_right_to_erasure_requirement(self, client, auth_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            with patch('app.api.v1.endpoints.compliance.gdpr_service') as mock_gdpr:
                mock_gdpr.initiate_data_erasure = AsyncMock(return_value={
    ...
    def test_gdpr_data_processing_consent_management(self, client, auth_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            response = client.get(
                "/api/v1/compliance/gdpr/consent-status",
                headers=auth_headers
            )
    ...
    def test_data_breach_notification_procedure(self, client, admin_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            with patch('app.api.v1.endpoints.compliance.breach_service') as mock_breach:
                mock_breach.report_data_breach = AsyncMock(return_value={
                ...
                breach_report = {
                    ...
                    "containment_actions": ["Systems isolated", "Passwords reset", "Forensic analysis initiated"]
                }
    ...
    def test_data_backup_and_recovery_procedures(self, client, admin_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            # Check backup status
            response = client.get(
                "/api/v1/compliance/backup-status",
                headers=admin_headers
            )
            ...
            assert "daily" in backup_frequency.lower() or "hourly" in backup_frequency.lower()
    ...
    def test_audit_log_retention_and_integrity(self, client, admin_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            assert integrity_status["status"] == "valid", "Audit logs must maintain integrity"
            ...
            if data["audit_events"]:
                for event in data["audit_events"]:
                    ...
                    assert "ip_address" in event, "Security auditing requires IP tracking"
    ...
    def test_professional_indemnity_coverage_validation(self, client, admin_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            response = client.get(
                "/api/v1/compliance/insurance-status",
                headers=admin_headers
            )
            ...
            expiry_date = datetime.fromisoformat(pi_coverage["expiry_date"].replace("Z", "+00:00"))
    ...
    def test_regulatory_compliance_monitoring(self, client, admin_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            response = client.get(
                "/api/v1/compliance/regulatory-status",
                headers=admin_headers
            )
            ...
            for regulation, status in data.items():
                if isinstance(status, dict):
                    ...
                    assert status["status"] == "compliant", f"{regulation} must be compliant"
    ...
    def test_incident_response_procedure(self, client, admin_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            with patch('app.api.v1.endpoints.compliance.incident_service') as mock_incident:
                mock_incident.create_incident = AsyncMock(return_value={
    ...
    def test_business_continuity_plan_validation(self, client, admin_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            response = client.get(
                "/api/v1/compliance/business-continuity-status",
                headers=admin_headers
            )
            ...
            assert "hour" in rto.lower() or "minute" in rto.lower(), "RTO should be in hours or better"
            assert "hour" in rpo.lower() or "minute" in rpo.lower(), "RPO should minimize data loss"
    ...
    def test_third_party_vendor_compliance(self, client, admin_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            response = client.get(
                "/api/v1/compliance/vendor-compliance",
                headers=admin_headers
            )
            ...
            for vendor in vendors:
                ...
                if any(critical in vendor["name"].lower() for critical in critical_vendors):
                    assert vendor["compliance_status"] == "compliant"
                    
                    # Should have relevant certifications
                    certifications = vendor["certifications"]
                    security_certs = ["SOC2", "ISO27001", "GDPR"]
                    has_security_cert = any(cert in certifications for cert in security_certs)
                    assert has_security_cert, f"Vendor {vendor['name']} should have security certifications"
    ...
    def test_data_retention_policy_compliance(self, client, admin_headers):
        ...
        with patch('app.api.v1.endpoints.compliance.get_current_user') as mock_user:
            ...
            response = client.get(
                "/api/v1/compliance/data-retention-status",
                headers=admin_headers
            )
            ...
            for data_type in data_types:
                if data_type in policies:
                    ...
                    assert "secure" in deletion.lower() or "encrypted" in deletion.lower()