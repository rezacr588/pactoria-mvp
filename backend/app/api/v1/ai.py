...
async def analyze_contract(
    ...
    try:
        ...
        return ContractAnalysisResponse(
            ...
            consumer_rights_compliance=compliance_data.get(
...
async def recommend_template(
    ...
    try:
        logger.info(
            f"Recommending template for: {request.business_description[:50]}..."
        )
...
async def assess_contract_risk(
    ...
    try:
        ...
        overall_risk = sum(factor["score"] for factor in risk_factors.values()) // len(
            risk_factors
        )
...
async def list_available_templates(
    category: str = None, current_user: dict = Depends(get_current_user)
):
    """
