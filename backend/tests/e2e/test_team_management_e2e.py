...
class TestTeamManagementE2E:
        ...
        try:
            ...
            for invitation_data in invitation_requests:
                invite_response = self.client.post(
                    "/api/v1/team/invite", json=invitation_data
                )
                ...
                if invite_response.status_code == 200:
                    ...
                    print(
                        f"Invitation failed for {invitation_data['email']}: {invite_response.json()}"
                    )
            ...
            members_response = self.client.get(
                "/api/v1/team/members?include_inactive=true"
            )
            ...
            admin_member = next(
                (m for m in all_members if m["email"] == admin_user.email), None
            )
            ...
            legal_filter_response = self.client.get(
                "/api/v1/team/members?department=Legal&include_inactive=true"
            )
            ...
            legal_count = len(
                [inv for inv in invited_members if inv.get("department") == "Legal"]
            )
            if legal_count > 0:
            ...
            role_change_response = self.client.put(
                f"/api/v1/team/members/{regular_user.id}/role", json={"role": "viewer"}
            )
            ...
            if regular_user:
                remove_response = self.client.delete(
                    f"/api/v1/team/members/{regular_user.id}"
                )
            ...
            self_remove_response = self.client.delete(
                f"/api/v1/team/members/{admin_user.id}"
            )
    ...
    def test_team_permission_enforcement(self):
        ...
        viewer_user = create_test_user(
            self.db, company.id, UserRole.VIEWER, email="viewer@test.com"
        )
        ...
        try:
            ...
            assert (
                "Only admin and contract manager users can invite"
                in invite_response.json()["detail"]
            )

            # Test viewer cannot change roles
            role_change_response = self.client.put(
                f"/api/v1/team/members/{admin_user.id}/role", json={"role": "viewer"}
            )
            assert role_change_response.status_code == 403
            assert (
                "Only admin users can update member roles"
                in role_change_response.json()["detail"]
            )
            ...
            assert (
                ...
                in remove_response.json()["detail"]
            )
    ...
    def test_team_size_limits(self):
        ...
        try:
            ...
            first_response = self.client.post(
                "/api/v1/team/invite", json=first_invitation
            )
            ...
            second_response = self.client.post(
                "/api/v1/team/invite", json=second_invitation
            )
    ...
    def test_duplicate_email_prevention(self):
        ...
        existing_user = create_test_user(
            self.db, company.id, UserRole.CONTRACT_MANAGER, email="existing@test.com"
        )
        ...
        try:
            ...
            response = self.client.post(
                "/api/v1/team/invite", json=duplicate_invitation
            )
    ...
    def test_error_handling_and_validation(self):
        ...
        try:
            ...
            response = self.client.post(
                "/api/v1/team/invite", json=invalid_email_invitation
            )
            ...
            response = self.client.post(
                "/api/v1/team/invite", json=incomplete_invitation
            )
            ...
            nonexistent_response = self.client.get(
                "/api/v1/team/members/nonexistent-id"
            )
...
class TestTeamManagementIntegration:
    ...
    def test_invitation_email_integration(self, mock_email_service):
        ...
        try:
            ...
            with patch(
                "app.application.services.team_application_service.EmailService",
                mock_email_service,
            ):
