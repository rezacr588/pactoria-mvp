...
class TestFileManagementFlows:
    ...
    def test_complete_file_upload_workflow(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        with performance_helper.measure_time("file_upload"):
            files = {"file": ("test_contract.txt", io.BytesIO(test_file_content), "text/plain")}
            form_data = {
        ...
        contract_files_response = client.get(f"/api/v1/files/?contract_id={contract.id}", headers=headers)
        ...
        deleted_download_response = client.get(f"/api/v1/files/{file_id}", headers=headers)
    ...
    def test_multiple_file_types_workflow(self, e2e_test_base: E2ETestBase):
        ...
        for test_file in test_files:
            files = {"file": (test_file["filename"], io.BytesIO(test_file["content"]), test_file["mime_type"])}
        ...
        for i, uploaded_file in enumerate(uploaded_files):
            download_response = client.get(f"/api/v1/files/{uploaded_file['file_id']}", headers=headers)
            e2e_test_base.assert_response_success(download_response)
        ...
        for uploaded_file in uploaded_files:
            unauthorized_download = client.get(f"/api/v1/files/{uploaded_file['file_id']}", headers=other_headers)
            e2e_test_base.assert_response_error(unauthorized_download, 403)
        
        # Cleanup: Delete uploaded files
        for uploaded_file in uploaded_files:
            delete_response = client.delete(f"/api/v1/files/{uploaded_file['file_id']}", headers=headers)
            e2e_test_base.assert_response_success(delete_response, 204)
            
    def test_file_validation_and_security(self, e2e_test_base: E2ETestBase, error_scenarios):
        ...
        files = {"file": ("large_file.txt", io.BytesIO(large_file_content), "text/plain")}
        
        large_file_response = client.post("/api/v1/files/upload", files=files, headers=headers)
        e2e_test_base.assert_response_error(large_file_response, 413)  # Request Entity Too Large
        
        # Test 2: Invalid file extension
        invalid_extension_content = b"Invalid file content"
        files = {"file": ("malicious.exe", io.BytesIO(invalid_extension_content), "application/octet-stream")}
        
        invalid_ext_response = client.post("/api/v1/files/upload", files=files, headers=headers)
        e2e_test_base.assert_response_error(invalid_ext_response, 400)  # Bad Request
        
        # Test 3: Invalid MIME type
        files = {"file": ("test.txt", io.BytesIO(b"test content"), "application/javascript")}  # JS not allowed
        ...
        empty_file_response = client.post("/api/v1/files/upload", files=files, headers=headers)
        ...
        files = {"file": ("../../../etc/passwd", io.BytesIO(malicious_content), "text/plain")}
        
        path_traversal_response = client.post("/api/v1/files/upload", files=files, headers=headers)
    ...
    def test_file_batch_operations(self, e2e_test_base: E2ETestBase):
        ...
        for i in range(5):
            ...
            files = {"file": (f"batch_file_{i+1}.txt", io.BytesIO(file_content), "text/plain")}
            form_data = {
        ...
        for batch_file in batch_files:
            delete_response = client.delete(f"/api/v1/files/{batch_file['file_id']}", headers=headers)
            e2e_test_base.assert_response_success(delete_response, 204)
...
class TestBulkOperationsFlows:
    ...
    def test_bulk_contract_update_workflow(self, e2e_test_base: E2ETestBase, bulk_operation_data, performance_helper):
        ...
        with performance_helper.measure_time("bulk_contract_update"):
            bulk_update_response = client.post("/api/v1/bulk/contracts/update", json=bulk_update_data, headers=headers)
        ...
        for i, contract in enumerate(contracts_to_update):
            updated_contract_response = client.get(f"/api/v1/contracts/{contract['id']}", headers=headers)
        ...
        performance_helper.assert_performance("bulk_contract_update", 5000)  # 5 seconds for 10 contracts
        
    def test_bulk_contract_export_workflow(self, e2e_test_base: E2ETestBase, bulk_operation_data):
        ...
        csv_export_data = {
            ...
            "format": "csv",
            "fields": ["title", "contract_type", "client_name", "contract_value", "status", "created_at"],
        ...
        excel_export_data = {
            ...
            "format": "excel",
            "fields": ["title", "contract_type", "supplier_name", "contract_value", "currency"],
            "include_content": False
        }
        ...
        json_export_data = {
            "contract_ids": [c["id"] for c in export_contracts if c["status"] == "active"],  # Filter
            "format": "json",
        ...
        pdf_export_response = client.post("/api/v1/bulk/contracts/export", json=pdf_export_data, headers=headers)
    ...
    def test_bulk_user_management_workflow(self, e2e_test_base: E2ETestBase, bulk_operation_data):
        ...
        bulk_invite_response = client.post("/api/v1/bulk/users/invite", json=invitation_data, headers=headers)
        ...
        bulk_role_response = client.put("/api/v1/bulk/users/role-change", json=role_change_data, headers=headers)
    ...
    def test_bulk_contract_delete_workflow(self, e2e_test_base: E2ETestBase):
        ...
        bulk_delete_response = client.post("/api/v1/bulk/contracts/delete", json=delete_data, headers=headers)
        ...
        for contract in active_contracts:
            assert contract["id"] not in deleted_ids, "Deleted contract still appears in active list"
        
        # Step 4: Verify individual contract access returns 404 or shows terminated status
        for contract in contracts_to_delete:
            individual_response = client.get(f"/api/v1/contracts/{contract['id']}", headers=headers)
    ...
    def test_bulk_operation_status_tracking(self, e2e_test_base: E2ETestBase):
        ...
        if "operation_id" in export_result:
            ...
            if status_response.status_code == 501:  # Not implemented
                ...
                assert "status" in status_data  # e.g., "PENDING", "PROCESSING", "COMPLETED"
...
class TestIntegratedFileAndBulkWorkflows:
    ...
    def test_bulk_contract_export_with_files(self, e2e_test_base: E2ETestBase):
        ...
        for i in range(3):
            ...
            contract_response = client.post("/api/v1/contracts/", json=contract_data, headers=headers)
            ...
            for j in range(2):  # 2 files per contract
                ...
                files = {"file": (f"contract_{i+1}_file_{j+1}.txt", io.BytesIO(file_content), "text/plain")}
                form_data = {
        ...
        for contract_with_files in contracts_with_files:
            for file_info in contract_with_files["files"]:
                delete_response = client.delete(f"/api/v1/files/{file_info['file_id']}", headers=headers)
                e2e_test_base.assert_response_success(delete_response, 204)
    ...
    def test_contract_template_file_management(self, e2e_test_base: E2ETestBase):
        ...
        files = {"file": ("template_guidance.pdf", io.BytesIO(template_file_content), "application/pdf")}
        form_data = {
        ...
        contract_file_content = b"This is a contract-specific document.\nAdditional terms and conditions."
        
        files = {"file": ("additional_terms.txt", io.BytesIO(contract_file_content), "text/plain")}
        form_data = {
