...
class TestContractCreationWorkflow:
    ...
    def contract_service(self, mock_contract_repository, mock_ai_service):
        ...
        return ContractApplicationService(
            contract_repository=mock_contract_repository, ai_service=mock_ai_service
        )

    @pytest.mark.asyncio
    async def test_complete_contract_creation_workflow(
    ...
    @pytest.mark.asyncio
    async def test_contract_creation_with_minimal_data(
        self, contract_service, mock_contract_repository
    ):
...
class TestContractContentGenerationWorkflow:
    ...
    def contract_service_with_mocks(self, existing_contract):
        ...
        return ContractApplicationService(
            contract_repository=mock_repository, ai_service=mock_ai_service
        )

    @pytest.mark.asyncio
    async def test_generate_contract_content_workflow(
        self, contract_service_with_mocks, existing_contract
    ):
        ...
        command = GenerateContractContentCommand(
            contract_id=existing_contract.id, regenerate=False
        )

        # Act
        updated_contract = await contract_service_with_mocks.generate_contract_content(
            command
        )
        ...
        content_generated_events = [
            e for e in domain_events if e.event_type == "ContractContentGenerated"
        ]
        assert len(content_generated_events) == 1
        assert content_generated_events[0].ai_model == "llama3-70b-8192"

    @pytest.mark.asyncio
    async def test_skip_generation_if_already_generated(
        self, contract_service_with_mocks, existing_contract
    ):
    ...
    @pytest.mark.asyncio
    async def test_force_regeneration_of_content(
        self, contract_service_with_mocks, existing_contract
    ):
        ...
        # Arrange - Set existing generated content
        existing_contract.set_generated_content(
            "OLD_CONTENT", {"model_name": "old-model", "confidence_score": 0.7}
        )

        command = GenerateContractContentCommand(
            contract_id=existing_contract.id, regenerate=True  # Force regeneration
        )
...
class TestContractComplianceAnalysisWorkflow:
    ...
    def compliance_service_with_mocks(self, contract_with_content):
        ...
        mock_ai_service.validate_contract_compliance = AsyncMock(
            return_value={
                ...
                "recommendations": [
                    "Add explicit GDPR consent clauses",
    ...
    @pytest.mark.asyncio
    async def test_compliance_analysis_workflow(
        self, compliance_service_with_mocks, contract_with_content
    ):
        ...
        command = AnalyzeComplianceCommand(
            contract_id=contract_with_content.id, force_reanalysis=False
        )

        # Act
        analyzed_contract = await compliance_service_with_mocks.analyze_compliance(
            command
        )
        ...
        assert (
            ...
            in analyzed_contract.risk_assessment.risk_factors
        )
        assert (
            "Add explicit GDPR consent clauses"
            in analyzed_contract.risk_assessment.recommendations
        )
        assert analyzed_contract.risk_assessment.risk_level == "Low"  # Score 1-3

    @pytest.mark.asyncio
    async def test_compliance_analysis_without_content_fails(
        ...
        compliance_service_with_mocks._contract_repository.get_by_id = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_skip_analysis_if_already_analyzed(
        self, compliance_service_with_mocks, contract_with_content
    ):
        ...
        existing_compliance = ComplianceScore(
            overall_score=0.85, gdpr_compliance=0.88, employment_law_compliance=0.82
        )
        ...
        contract_with_content.set_compliance_analysis(
            existing_compliance, existing_risk
        )
...
class TestContractStatusTransitionWorkflows:
    ...
    @pytest.mark.asyncio
    async def test_activate_contract_workflow(
        self, status_transition_service, draft_contract_with_content
    ):
        ...
        activation_events = [
            e for e in domain_events if e.event_type == "ContractActivated"
        ]
        assert len(activation_events) == 1
        assert activation_events[0].activated_by_user_id == "manager-456"

    @pytest.mark.asyncio
    async def test_activate_contract_without_content_fails(
        ...
        status_transition_service._contract_repository.get_by_id = AsyncMock(
            return_value=incomplete_contract
        )

        command = ActivateContractCommand(
            contract_id=contract_id, activated_by_user_id="manager-456"
        )
        ...
        assert (
            "cannot activate contract without final content" in str(exc.value).lower()
        )
    ...
    async def test_complete_active_contract_workflow(self, status_transition_service):
        ...
        completion_events = [
            e for e in domain_events if e.event_type == "ContractCompleted"
        ]
        assert len(completion_events) == 1
        assert completion_events[0].completed_by_user_id == "manager-789"
        assert (
            completion_events[0].completion_reason == "Project completed successfully"
        )

    @pytest.mark.asyncio
    async def test_terminate_contract_workflow(
        self, status_transition_service, draft_contract_with_content
    ):
        ...
        # Act
        terminated_contract = await status_transition_service.terminate_contract(
            command
        )
        ...
        termination_events = [
            e for e in domain_events if e.event_type == "ContractTerminated"
        ]
    ...
    @pytest.mark.asyncio
    async def test_terminate_without_reason_fails(
        self, status_transition_service, draft_contract_with_content
    ):
...
class TestContractVersionManagementWorkflow:
    ...
    async def test_contract_version_creation_workflow(self, versioned_contract):
        ...
        assert (
            ...
            == "Updated payment terms and added liability clauses"
        )
    ...
    @pytest.mark.asyncio
    async def test_prevent_version_creation_on_completed_contract(
...
class TestContractExpirationAndRenewalWorkflows:
    ...
    def expiring_contract(self):
        ...
        contract.set_date_range(
            DateRange(
                ...
                + timedelta(days=5),  # Expires in 5 days
            )
        )
    ...
    async def test_contract_expiration_detection(self, expiring_contract):
        ...
        days_until_expiry = (
            expiring_contract.date_range.end_date - datetime.now(timezone.utc)
        ).days
    ...
    async def test_expired_contract_detection(self):
        ...
        # Set past date range
        expired_contract.set_date_range(
            DateRange(
                start_date=datetime.now(timezone.utc) - timedelta(days=60),
                end_date=datetime.now(timezone.utc)
                - timedelta(days=10),  # Expired 10 days ago
            )
        )
...
class TestContractCollaborationWorkflow:
    ...
    def collaborative_contract(self):
        ...
        supplier = ContractParty(
            "Collaborative Supplier", Email("supplier@example.com")
        )
    ...
    async def test_multi_user_contract_workflow(self, collaborative_contract):
        ...
        compliance_score = ComplianceScore(
            overall_score=0.91, gdpr_compliance=0.94, employment_law_compliance=0.88
        )
        ...
        collaborative_contract.set_compliance_analysis(
