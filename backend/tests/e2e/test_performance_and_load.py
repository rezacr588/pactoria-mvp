...
class TestAPIPerformance:
    ...
    def test_contract_crud_performance(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        for i in range(20):
            contract_data = {k: v.format(i+1) if isinstance(v, str) else v for k, v in contract_data_template.items()}
        ...
        with performance_helper.measure_time("contract_listing"):
            list_response = client.get("/api/v1/contracts/?page=1&size=20", headers=headers)
        ...
        for contract in created_contracts[:5]:  # Test subset for deletion
            with performance_helper.measure_time("contract_deletion"):
                delete_response = client.delete(f"/api/v1/contracts/{contract['id']}", headers=headers)
    ...
    def test_ai_operations_performance(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        performance_helper.assert_performance("ai_compliance_analysis", 5000)   # 5 seconds
        performance_helper.assert_performance("ai_regeneration", 10000)         # 10 seconds
        
    def test_search_and_filtering_performance(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        search_queries = ["Contract", "project", "consulting", "Search Client 050", "Supplier Company"]
        
        for query in search_queries:
            with performance_helper.measure_time("basic_search"):
                search_response = client.get(f"/api/v1/contracts/?search={query}", headers=headers)
        ...
        for filter_query in filter_combinations:
            with performance_helper.measure_time("filtered_search"):
                filter_response = client.get(f"/api/v1/contracts/{filter_query}", headers=headers)
        ...
        for page in range(1, 6):  # Test first 5 pages
            with performance_helper.measure_time("paginated_search"):
                page_response = client.get(f"/api/v1/contracts/?page={page}&size=20", headers=headers)
        ...
        with performance_helper.measure_time("advanced_search"):
            advanced_response = client.post("/api/v1/search/contracts", json=advanced_search_data, headers=headers)
    ...
    def test_analytics_performance(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        performance_helper.assert_performance("time_series_analytics", 1500)   # 1.5 seconds


class TestConcurrentUserLoad:
    """Tests simulating multiple concurrent users"""
    
    def test_concurrent_contract_operations(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        # Performance assertions
        performance_helper.assert_performance("concurrent_contract_creation", 3000)  # 3 seconds per creation
        performance_helper.assert_performance("concurrent_read", 1000)               # 1 second per read
        performance_helper.assert_performance("concurrent_write", 3000)              # 3 seconds per write
        
    def test_high_frequency_api_requests(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        if test_contracts:
            ...
            for i in range(50):  # 50 rapid detail requests
                ...
                with performance_helper.measure_time("rapid_detail_request"):
                    response = client.get(f"/api/v1/contracts/{contract['id']}", headers=headers)
        ...
        if test_contracts:
            mixed_operations.append(
                lambda: client.get(f"/api/v1/contracts/{test_contracts[0]['id']}", headers=headers)
            )
        ...
        performance_helper.assert_performance("mixed_burst_request", 1500)    # 1.5 seconds


class TestDataScalePerformance:
    """Tests performance with large datasets"""
    
    def test_large_contract_dataset_performance(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        for batch in range(10):  # 10 batches of 50 = 500 contracts
            ...
            for i in range(batch_size):
                ...
                contract_data = {
                    ...
                    "contract_type": contract_types[contract_index % len(contract_types)],
                ...
                response = client.post("/api/v1/contracts/", json=contract_data, headers=headers)
                if response.status_code != 201:
                    print(f"Failed to create contract {contract_index+1}: {response.status_code}")
        ...
        with performance_helper.measure_time("large_dataset_listing"):
            list_response = client.get("/api/v1/contracts/?page=1&size=50", headers=headers)
            e2e_test_base.assert_response_success(list_response)
        ...
        for page in [1, 5, 10, 15, 20]:  # Test various pages
            with performance_helper.measure_time("large_dataset_pagination"):
                page_response = client.get(f"/api/v1/contracts/?page={page}&size=25", headers=headers)
                e2e_test_base.assert_response_success(page_response)
        
        # Performance assertions for large dataset
        performance_helper.assert_performance("large_dataset_listing", 2000)      # 2 seconds
        performance_helper.assert_performance("large_dataset_search", 3000)       # 3 seconds
        performance_helper.assert_performance("large_dataset_filtering", 2500)    # 2.5 seconds
        performance_helper.assert_performance("large_dataset_analytics", 5000)    # 5 seconds
        performance_helper.assert_performance("large_dataset_pagination", 1500)   # 1.5 seconds
        
    def test_bulk_operations_performance(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        with performance_helper.measure_time("small_bulk_update"):
            small_bulk_response = client.post("/api/v1/bulk/contracts/update", json=small_bulk_data, headers=headers)
        ...
        with performance_helper.measure_time("medium_bulk_update"):
            medium_bulk_response = client.post("/api/v1/bulk/contracts/update", json=medium_bulk_data, headers=headers)
        ...
        with performance_helper.measure_time("large_bulk_update"):
            large_bulk_response = client.post("/api/v1/bulk/contracts/update", json=large_bulk_data, headers=headers)
        ...
        with performance_helper.measure_time("bulk_export"):
            export_response = client.post("/api/v1/bulk/contracts/export", json=export_data, headers=headers)
        ...
        with performance_helper.measure_time("bulk_delete"):
            delete_response = client.post("/api/v1/bulk/contracts/delete", json=delete_data, headers=headers)
            e2e_test_base.assert_response_success(delete_response)
        
        # Performance assertions for bulk operations
        performance_helper.assert_performance("small_bulk_update", 3000)    # 3 seconds for 10
        performance_helper.assert_performance("medium_bulk_update", 8000)   # 8 seconds for 25
        performance_helper.assert_performance("large_bulk_update", 15000)   # 15 seconds for 50
...
class TestMemoryAndResourceUsage:
    ...
    def test_memory_usage_under_load(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        contracts_created = []
        for i in range(50):
            large_content = f"This is a large contract content for memory testing. " * 100  # Large content
        ...
        import io
        files = {"file": ("large_memory_test.txt", io.BytesIO(large_file_content), "text/plain")}
        ...
        performance_helper.assert_performance("memory_intensive_analytics", 10000) # 10 seconds
        performance_helper.assert_performance("memory_intensive_export", 15000)    # 15 seconds
...
class TestPerformanceBenchmarks:
    ...
    def test_api_response_time_benchmarks(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        import io
        files = {"file": ("benchmark.txt", io.BytesIO(b"Benchmark test file content"), "text/plain")}
        upload_response = client.post("/api/v1/files/upload", files=files, headers=headers)
        file_data = upload_response.json() if upload_response.status_code == 201 else None
        
        # Define benchmark tests
        benchmark_operations = [
            ("auth_profile", lambda: client.get("/api/v1/auth/me", headers=headers), 200),
            ("contract_list", lambda: client.get("/api/v1/contracts/?page=1&size=10", headers=headers), 100),
            ("contract_get", lambda: client.get(f"/api/v1/contracts/{contract.id}", headers=headers), 300),
            ("contract_create", lambda: client.post("/api/v1/contracts/", json={
                "title": "Benchmark Contract",
                "contract_type": "service_agreement", 
                "plain_english_input": "Benchmark test contract"
            }, headers=headers), 2000),
            ("analytics_dashboard", lambda: client.get("/api/v1/analytics/dashboard", headers=headers), 2000),
            ("search_basic", lambda: client.get("/api/v1/contracts/?search=test", headers=headers), 1000),
        ]
        
        if file_data:
            benchmark_operations.extend([
                ("file_list", lambda: client.get("/api/v1/files/", headers=headers), 500),
                ("file_download", lambda: client.get(f"/api/v1/files/{file_data['file_id']}", headers=headers), 1000)
            ])
        ...
        for operation_name, operation_func, target_ms in benchmark_operations:
            ...
            if times:
                ...
                print(f"{operation_name}: avg={avg_time:.2f}ms, target={target_ms}ms, {'✓' if avg_time <= target_ms else '✗'}")
    ...
    def test_scalability_projections(self, e2e_test_base: E2ETestBase, performance_helper):
        ...
        for size in dataset_sizes:
            ...
            operations = {
                "list_contracts": lambda: client.get("/api/v1/contracts/?page=1&size=20", headers=headers),
                "search_contracts": lambda: client.get("/api/v1/contracts/?search=Scaling", headers=headers),
                "analytics": lambda: client.get("/api/v1/analytics/business", headers=headers)
            }
