...
class LegalClause(AggregateRoot[ClauseId]):
    ...
    # Business operations
    def update_content(
        self, new_content: str, user_id: str, version_increment: str = "patch"
    ):
    ...
    def add_relationship(self, relationship: ClauseRelationship):
        ...
        existing = any(
            r.related_clause_id == relationship.related_clause_id
    ...
    def is_compatible_with_clause(self, other_clause_id: str) -> bool:
        ...
        conflicts = [
            r
            for r in self._relationships
            if r.related_clause_id == other_clause_id
            and r.relationship_type == "conflicts"
        ]
    ...
    def get_quality_score(self) -> float:
        ...
        rating_component = (
            (self.average_rating - 1) / 4 if self.average_rating > 0 else 0.5
        )
    ...
    def _validate_title(self, title: str):
        ...
        if len(title.strip()) > 200:
            raise DomainValidationError(
                "Clause title too long (max 200 characters)", "title", title
            )
    ...
    def _validate_content(self, content: str):
        ...
        if not content or not content.strip():
            raise DomainValidationError(
                "Clause content cannot be empty", "content", content
            )

        if len(content) < 50:
            raise DomainValidationError(
                "Clause content too short (min 50 characters)", "content", content
            )
    ...
    def _detect_compliance_frameworks(self) -> List[ComplianceFramework]:
        ...
        if any(
            ...
            for term in ["personal data", "gdpr", "data protection", "privacy"]
        ):
        ...
        if any(
            term in content_lower
            for term in ["employee", "employment", "termination", "notice period"]
        ):
        ...
        if any(
            term in content_lower
            for term in ["consumer", "unfair terms", "consumer rights"]
        ):
            frameworks.append(ComplianceFramework.CONSUMER_RIGHTS)

        # Financial Services
        if any(
            term in content_lower for term in ["financial", "fca", "regulated activity"]
        ):
        ...
        if any(
            term in content_lower
            for term in ["competition", "anti-competitive", "market abuse"]
        ):
    ...
    def _content_significantly_changed(
    ...
    def _validate_variable_value(
        self, variable: ClauseVariable, value: Any
    ) -> List[str]:
        ...
        if isinstance(value, str):
            if min_length and len(value) < min_length:
                errors.append(
                    f"Variable '{variable.display_name}' too short (min {min_length} chars)"
                )
            if max_length and len(value) > max_length:
                errors.append(
                    f"Variable '{variable.display_name}' too long (max {max_length} chars)"
                )
...
class ClauseLibrary(AggregateRoot[str]):
    ...
    def search_clauses(
        self, query: str, category: Optional[ClauseCategory] = None
    ) -> List[LegalClause]:
        ...
        for clause in self._clauses.values():
            ...
            # Text search
            if (
                query_lower in clause.title.lower()
                or query_lower in clause.content.lower()
                or (clause.description and query_lower in clause.description.lower())
            ):
