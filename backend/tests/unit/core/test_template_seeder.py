...
from app.core.template_seeder import TEMPLATE_DATA, seed_templates, async_seed_templates
...
class TestTemplateData:
    ...
    def test_template_data_structure(self):
        ...
        for template in TEMPLATE_DATA:
            for field in required_fields:
                assert (
                    field in template
                ), f"Missing field {field} in template {template.get('name', 'unknown')}"
    ...
    def test_template_content_has_placeholders(self):
        ...
        for template in TEMPLATE_DATA:
            ...
            ), f"Template {template['name']} missing placeholders"
    ...
    def test_compliance_features_not_empty(self):
        ...
        for template in TEMPLATE_DATA:
            ...
            assert (
                len(compliance) > 0
            ), f"Template {template['name']} has no compliance features"
            ...
            assert any(
                indicator in compliance_text for indicator in uk_indicators
...
class TestTemplateValidation:
    ...
    def test_all_contract_types_covered(self):
        ...
        for expected_type in expected_types:
            assert (
                expected_type in template_types
            ), f"Missing template for {expected_type}"
    ...
    def test_template_content_length(self):
        ...
        for template in TEMPLATE_DATA:
            ...
            assert (
                content.count("\n") > 5
            ), f"Template {template['name']} lacks structure"
    ...
    def test_uk_legal_compliance_keywords(self):
        ...
        for template in TEMPLATE_DATA:
            ...
            assert (
                has_uk_keywords
            ), f"Template {template['name']} lacks UK legal compliance keywords"
