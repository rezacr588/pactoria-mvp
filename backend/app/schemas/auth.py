...
class UserRegister(BaseModel):
    ...
    email: EmailStr = Field(..., description="User email address", example="jane.smith@techstartup.co.uk")
    password: str = Field(..., min_length=8, description="User password (minimum 8 characters)", example="MyStrongPass456!")
    full_name: str = Field(..., min_length=2, max_length=100, description="User's full name", example="Jane Smith")
    company_name: Optional[str] = Field(None, max_length=100, description="Company name (optional)", example="Tech Startup Ltd")
    timezone: Optional[str] = Field("Europe/London", description="User timezone", example="Europe/London")
...
class UserProfile(BaseModel):
    ...
    id: str = Field(..., description="Unique user identifier", example="user_123e4567-e89b-12d3-a456-426614174000")
    email: str = Field(..., description="User email address", example="john.doe@smecompany.co.uk")
    ...
    company_id: Optional[str] = Field(None, description="Associated company ID", example="comp_987fcdeb-51a2-43d7-8f19-123456789abc")
    created_at: datetime = Field(..., description="Account creation timestamp", example="2024-01-15T09:30:00Z")
    last_login_at: Optional[datetime] = Field(None, description="Last login timestamp", example="2024-08-31T14:20:30Z")
