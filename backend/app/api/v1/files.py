...
@router.post(
    ...
    **Requires Authentication:** JWT Bearer token
    ...
    responses={
        ...
        400: {
            "description": "Invalid file type or content",
            "model": ErrorResponse
        },
        401: {
...
async def list_files(
    ...
    if contract_id:
        query = query.filter(
            AuditLog.new_values['contract_id'].astext == contract_id
        )
    ...
    # Apply pagination
    offset = (page - 1) * size
    upload_logs = query.order_by(AuditLog.timestamp.desc()).offset(offset).limit(size).all()
    ...
    for log in upload_logs:
        if log.new_values:
            files.append(FileUploadResponse(
                ...
                original_filename=log.new_values.get("original_filename", "unknown"),
                file_size=log.new_values.get("file_size", 0),
                mime_type=log.new_values.get("mime_type", "application/octet-stream"),
...
async def delete_file(
    ...
    delete_log = AuditLog(
        ...
        additional_data={
            "original_upload_time": upload_log.timestamp.isoformat()
        }
    )
    db.add(delete_log)
    db.commit()