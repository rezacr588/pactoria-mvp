...
class TestTeamDomainLogic:
    ...
    def test_user_creation_with_company(self):
        ...
        manager = create_test_user(
            self.db, company.id, UserRole.CONTRACT_MANAGER, email="manager@test.com"
        )
    ...
    def test_team_invitation_creation(self):
        ...
        db_invitation = (
            ...
            .filter(TeamInvitation.email == "newuser@test.com")
            .first()
        )
    ...
    def test_company_user_limits(self):
        ...
        user2 = create_test_user(
            self.db, company.id, UserRole.CONTRACT_MANAGER, email="user2@test.com"
        )
        user3 = create_test_user(
            self.db, company.id, UserRole.VIEWER, email="user3@test.com"
        )
    ...
    def test_user_filtering_by_role_and_department(self):
        ...
        admin = create_test_user(
            self.db, company.id, UserRole.ADMIN, department="Legal"
        )
        ...
        managers = (
            self.db.query(User)
            .filter(
                User.company_id == company.id, User.role == UserRole.CONTRACT_MANAGER
            )
            .all()
        )
        ...
        legal_users = (
            self.db.query(User)
            .filter(User.company_id == company.id, User.department == "Legal")
            .all()
        )
    ...
    def test_subscription_tier_limits(self):
        ...
        starter_company = create_test_company(
            self.db, subscription_tier=SubscriptionTier.STARTER, max_users=5
        )
        professional_company = create_test_company(
            self.db, subscription_tier=SubscriptionTier.PROFESSIONAL, max_users=20
        )
        business_company = create_test_company(
            self.db, subscription_tier=SubscriptionTier.BUSINESS, max_users=100
        )
...
class TestTeamInvitationDomain:
    ...
    def test_invitation_status_transitions(self):
        ...
        db_invitation = (
            self.db.query(TeamInvitation)
            .filter(TeamInvitation.email == "transition@test.com")
            .first()
        )
