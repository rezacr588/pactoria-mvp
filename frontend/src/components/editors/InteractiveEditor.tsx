...
import { ContractFormField } from '../forms/ContractFormField';
import {
  CheckCircleIcon,
  ArrowPathIcon,
  DocumentTextIcon,
...
interface InteractiveEditorProps {
  data: EditorFormData;
  onUpdate: (data: Partial<EditorFormData>) => Promise<void>;
  isUpdating: boolean;
  onCancel: () => void;
  errors?: FormErrors;
...
export function InteractiveEditor({ 
  ...
  const [formData, setFormData] = useState<EditorFormData>(data);
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.name.trim()) {
      newErrors.name = `${mode === 'template' ? 'Template' : 'Contract'} name is required`;
    }
    ...
    if (mode === 'template') {
      ...
      if (!formData.category?.trim()) {
        newErrors.category = 'Template category is required';
      }
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      ...
      <div className="card-elevated">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <ContractFormField
            label={`${mode === 'template' ? 'Template' : 'Contract'} Name`}
            name="name"
            type="text"
            value={formData.name}
            ...
            required
            placeholder={mode === 'template' ? 'e.g., Employment Contract Template' : 'e.g., Marketing Services - Acme Corp'}
            tooltip={`Choose a descriptive name that helps you identify this ${mode} easily`}
          ...
          {mode === 'template' && (
            <>
              <ContractFormField
                ...
                required
                tooltip="Select the appropriate legal category for this template"
                showTooltip={showTooltip}
                onTooltipClick={handleTooltip}
              >
              ...
              <ContractFormField
                ...
                onChange={handleInputChange}
                error={allErrors.contractType}
                required
                tooltip="Specify the specific type of contract this template creates"
                showTooltip={showTooltip}
                onTooltipClick={handleTooltip}
              >
          ...
          <div className="lg:col-span-2">
            <ContractFormField
              label={`${mode === 'template' ? 'Template' : 'Contract'} Description`}
              name="description"
              type="textarea"
              value={formData.description}
              onChange={handleInputChange}
              placeholder={`Brief description of the ${mode} purpose and scope (optional)`}
              helpText={`Provide a high-level overview of what this ${mode} covers`}
              rows={3}
            />
          </div>
        </div>
      ...
      {/* Template Content Section (Template mode only) */}
      {mode === 'template' && (
        <div className="card-elevated">
          <div className="flex items-center mb-6">
          ...
          <div className="space-y-6">
            <ContractFormField
              ...
              onChange={handleInputChange}
              error={allErrors.templateContent}
              required
              placeholder="Enter the template content with variables like {{client_name}}, {{contract_value}}, etc."
              tooltip="Use double curly braces {{variable_name}} for dynamic content that will be filled in when creating contracts"
              showTooltip={showTooltip}
              onTooltipClick={handleTooltip}
              rows={10}
            ...
            <ContractFormField
              ...
              onChange={handleInputChange}
              placeholder="Important legal considerations and usage guidelines for this template"
              tooltip="Include any important legal notes or restrictions for using this template"
              showTooltip={showTooltip}
              onTooltipClick={handleTooltip}
              rows={3}
            />
            ...
            <ContractFormField
              ...
              onChange={handleInputChange}
              placeholder="1.0"
              tooltip="Template version for tracking changes and updates"
              showTooltip={showTooltip}
              onTooltipClick={handleTooltip}
            />
          </div>
      ...
      {/* Parties Section (Contract mode only) */}
      {mode === 'contract' && (
        <div className="card-elevated">
          <div className="flex items-center mb-6">
      ...
      {/* Contract Terms Section (Contract mode only) */}
      {mode === 'contract' && (
        <div className="card-elevated">
          <div className="flex items-center mb-6">
            <ClipboardDocumentListIcon className="h-6 w-6 text-primary-500 mr-3" />
      ...
      <div className="card-elevated">
        <div className="flex items-center mb-6">
          ...
          <h3 className={`text-xl font-semibold ${textColors.primary}`}>
            {mode === 'template' ? 'Template Metadata' : 'Dates & Payment Terms'}
          </h3>
        </div>
            ...
            <div className="relative flex items-center">
              {isUpdating ? (
                <span className="flex items-center">
                  ...
                  {mode === 'template' ? 'Updating Template...' : 'Updating Contract...'}
                </span>
              ) : (
                <span className="flex items-center">
                  <CheckCircleIcon className="h-5 w-5 mr-2 group-hover:scale-110 transition-transform" />
                  {mode === 'template' ? 'Update Template' : 'Update Contract'}
                </span>
              )}
            </div>
          </button>
