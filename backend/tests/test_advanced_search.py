...
def test_contracts():
    ...
    for i in range(10):
        contract = Contract(
            ...
            contract_type=(
                ContractType.SERVICE_AGREEMENT if i % 2 == 0 else ContractType.NDA
            ),
...
class TestAdvancedContractSearch:
    ...
    def test_advanced_search_should_support_text_queries(
        self, client, mock_db, test_user, test_contracts
    ):
        ...
        # Mock dependencies
        with patch("app.core.database.get_db", return_value=mock_db), patch(
            "app.core.auth.get_current_user", return_value=test_user
        ):
            ...
            matching_contracts = [
                ...
                if "Service" in str(c.plain_english_input)
                and "Client" in str(c.client_name)
            ]
            ...
            assert all(
                "Service" in item["plain_english_input"] for item in result["items"]
            )

    def test_advanced_search_should_support_date_range_filters(
        self, client, mock_db, test_user, test_contracts
    ):
    ...
    def test_advanced_search_should_support_numeric_range_filters(
        self, client, mock_db, test_user, test_contracts
    ):
        ...
        search_request = {
            "query": "",
            "filters": {"contract_value": {"gte": 2000.0, "lte": 8000.0}},
        }
        ...
        with patch("app.core.database.get_db", return_value=mock_db), patch(
            ...
            assert all(
                2000.0 <= item["contract_value"] <= 8000.0 for item in result["items"]
            )

    def test_advanced_search_should_support_status_filtering(
        self, client, mock_db, test_user, test_contracts
    ):
        """Test filtering by contract status"""
        # Given
        search_request = {"query": "", "filters": {"status": ["ACTIVE", "DRAFT"]}}
        ...
        with patch("app.core.database.get_db", return_value=mock_db), patch(
            ...
            status_filtered_contracts = [
                c for c in test_contracts if c.status.value in ["ACTIVE", "DRAFT"]
            ]
            mock_db.query.return_value.filter.return_value.all.return_value = (
                status_filtered_contracts
            )
            ...
            assert all(
                item["status"] in ["ACTIVE", "DRAFT"] for item in result["items"]
            )

    def test_advanced_search_should_support_sorting(
        self, client, mock_db, test_user, test_contracts
    ):
        ...
        with patch("app.core.database.get_db", return_value=mock_db), patch(
            ...
            sorted_contracts = sorted(
                test_contracts, key=lambda x: (-x.contract_value, x.created_at)
            )
    ...
    def test_advanced_search_should_support_pagination(
        self, client, mock_db, test_user, test_contracts
    ):
        """Test search results pagination"""
        # Given
        search_request = {"query": "", "filters": {}, "page": 2, "size": 3}
    ...
    def test_advanced_search_should_support_field_selection(
        self, client, mock_db, test_user, test_contracts
    ):
    ...
    def test_advanced_search_should_respect_company_isolation(
        ...
        search_request = {"query": "test", "filters": {}}

        # Mock dependencies - user from different company
        with patch("app.core.database.get_db", return_value=mock_db), patch(
            "app.core.auth.get_current_user", return_value=other_company_user
        ):

            # Should only return contracts from user's company
            mock_db.query.return_value.filter.return_value.all.return_value = (
                []
            )  # No contracts for other company
...
class TestUserSearch:
    ...
    def test_user_search_should_find_by_email_and_name(
        self, client, mock_db, test_user
    ):
        ...
        search_request = {"query": "john", "filters": {}}
        ...
        with patch("app.core.database.get_db", return_value=mock_db), patch(
            ...
            matching_users = [
                u
                for u in users
                if "john" in u.email.lower() or "john" in u.full_name.lower()
            ]
            mock_db.query.return_value.filter.return_value.all.return_value = (
                matching_users
            )
    ...
    def test_user_search_should_filter_by_role(self, client, mock_db, test_user):
        ...
        search_request = {
            "query": "",
            "filters": {"role": ["ADMIN", "CONTRACT_MANAGER"]},
        }
        ...
        with patch("app.core.database.get_db", return_value=mock_db), patch(
            ...
            assert all(
                item["role"] in ["ADMIN", "CONTRACT_MANAGER"]
                for item in result["items"]
            )
...
class TestTemplateSearch:
    ...
    def test_template_search_should_find_by_name_and_category(
        self, client, mock_db, test_user
    ):
        ...
        search_request = {"query": "service", "filters": {}}
        ...
        with patch("app.core.database.get_db", return_value=mock_db), patch(
            ...
            matching_templates = [
                t
                for t in templates
                if "service" in t.name.lower() or "service" in t.category.lower()
            ]
            mock_db.query.return_value.filter.return_value.all.return_value = (
                matching_templates
            )
    ...
    def test_template_search_should_filter_by_contract_type(
        self, client, mock_db, test_user
    ):
        ...
        search_request = {
            "query": "",
            "filters": {"contract_type": ["SERVICE_AGREEMENT", "NDA"]},
        }
        ...
        with patch("app.core.database.get_db", return_value=mock_db), patch(
            ...
            assert result["total"] == 2
            assert all(
                item["contract_type"] in ["SERVICE_AGREEMENT", "NDA"]
                for item in result["items"]
            )
...
class TestSearchValidation:
    ...
    def test_search_should_validate_query_length(self, client, mock_db, test_user):
        ...
        search_request = {"query": "a", "filters": {}}  # Too short
