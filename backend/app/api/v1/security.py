...
async def get_security_summary(
    hours: int = Query(24, ge=1, le=168), current_user: User = Depends(get_admin_user)
):
...
async def check_compliance(
    ...
    if compliance_request.resource_type == "contract":
        contract = (
            db.query(Contract)
            .filter(Contract.id == compliance_request.resource_id)
            .first()
        )
        if not contract:
            raise HTTPException(
        ...
        compliance_score = (
            ...
            .filter(ComplianceScore.contract_id == contract.id)
            .order_by(desc(ComplianceScore.analysis_date))
            .first()
        )
            ...
            for risk_factor in compliance_score.risk_factors:
                issues.append(
                    ComplianceIssue(
                        ...
                        severity=(
                            "medium" if compliance_score.risk_score >= 5 else "low"
                        ),
        ...
        if "UK_EMPLOYMENT_LAW" in compliance_request.compliance_standards:
            if contract.contract_type.value == "employment_contract":
                if (
                    not contract.final_content
                    or "notice period" not in contract.final_content.lower()
                ):
                    issues.append(
                        ComplianceIssue(
        ...
        company = (
            db.query(Company)
            .filter(Company.id == compliance_request.resource_id)
            .first()
        )
        if not company:
            raise HTTPException(
...
async def create_gdpr_data_request(
    ...
    # Generate request ID
    request_id = (
        f"gdpr-{gdpr_request.request_type}-{int(datetime.utcnow().timestamp())}"
    )
    ...
    if gdpr_request.request_type == "access":
        ...
        user_contracts = db.query(Contract).filter(Contract.created_by == user.id).all()
    ...
    if not current_user.is_admin:
        if not current_user.company_id:
            ...
            company_contracts = (
                ...
                .filter(Contract.company_id == current_user.company_id)
                .subquery()
            )
...
async def get_security_config(current_user: User = Depends(get_admin_user)):
...
async def get_threat_intelligence(current_user: User = Depends(get_admin_user)):
    ...
    return ThreatIntelligenceResponse(
        ...
        suspicious_patterns_detected=summary["event_types"].get(
