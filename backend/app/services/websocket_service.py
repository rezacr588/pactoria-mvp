...
from app.infrastructure.database.models import User, Contract
from app.schemas.websocket import (
    WebSocketMessage, MessageType, ContractUpdateMessage, NotificationMessage,
    SystemMessage, ConnectionMessage, ErrorMessage, UserJoinedMessage,
    UserLeftMessage, BulkOperationMessage, AnyWebSocketMessage
)
...
class WebSocketManager:
    ...
    async def connect(self, websocket: WebSocket, user_id: str, company_id: str) -> str:
        ...
        try:
            ...
            connection_msg = ConnectionMessage.connected(user_id, company_id, session_id)
    ...
    async def disconnect(self, session_id: str, reason: str = "Connection closed"):
        ...
        try:
            ...
            logger.info(f"WebSocket disconnected: session_id={session_id}, reason={reason}")
        ...
        for session_id in user_sessions.copy():
            ...
            if connection and connection.is_active:
                try:
                    ...
                    logger.error(f"Failed to send to user {user_id}, session {session_id}: {e}")
        ...
        for session_id in company_sessions.copy():
            ...
            if connection and connection.is_active:
                try:
                    ...
                    logger.error(f"Failed to broadcast to company {company_id}, session {session_id}: {e}")
    ...
    async def handle_message(self, session_id: str, message: Dict[str, Any]):
        ...
        try:
            ...
            if message_type == MessageType.PING:
                pong_msg = {"type": MessageType.PONG, "timestamp": datetime.now(timezone.utc).isoformat()}
    ...
    async def _validate_connection_limits(self, user_id: str, company_id: str):
        ...
        user_active_sessions = sum(
            1 for session_id in self.user_sessions.get(user_id, set())
            if session_id in self.connections and self.connections[session_id].is_active
        )
        ...
        company_active_sessions = sum(
            1 for session_id in self.company_sessions.get(company_id, set())
            if session_id in self.connections and self.connections[session_id].is_active
        )
    ...
    async def _send_to_connection(self, connection: WebSocketConnection, message: Dict[str, Any]):
        ...
        try:
            ...
            logger.error(f"Failed to send message to connection {connection.session_id}: {e}")
...
async def send_user_notification(message: NotificationMessage):
    ...
    if message.target_user_id:
        return await websocket_manager.send_to_user(message.target_user_id, message.dict())
    return 0
...
async def notify_users_by_role(company_id: str, roles: List[str], message: Dict[str, Any]):
...
async def send_bulk_operation_update(operation_id: str, message: BulkOperationMessage, user_id: str):
    """Send bulk operation progress update to user"""
    return await websocket_manager.send_to_user(user_id, message.dict())


async def batch_send_notifications(user_id: str, notifications: List[NotificationMessage]):
