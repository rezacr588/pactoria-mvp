...
class DataFactories:
    ...
    def generate_company_data(self, **overrides) -> Dict[str, Any]:
        ...
        data = {
            ...
            "settings": {
                ...
                "branding": {"primary_color": self.fake.hex_color(), "logo_url": None},
            },
        }
    ...
    def generate_user_data(
        self, company_id: str = None, role: UserRole = None, **overrides
    ) -> Dict[str, Any]:
        ...
        data = {
            ...
            "invited_at": (
                self.fake.date_time_between(start_date="-60d", end_date="-30d")
    ...
    def generate_contract_data(
        self, company_id: str, created_by: str, template_id: str = None, **overrides
    ) -> Dict[str, Any]:
    ...
    def _get_contract_type_specific_data(
    ...
    def generate_template_data(
        self, contract_type: ContractType = None, **overrides
    ) -> Dict[str, Any]:
...
[SIGNATURE_BLOCKS]
    ...
    def _get_compliance_features(self, contract_type: ContractType) -> List[str]:
        ...
        type_specific = {
            ...
            ContractType.CONSUMER_AGREEMENT: [
                "consumer_rights",
                "unfair_terms",
                "distance_selling",
            ],
    ...
    def generate_compliance_score_data(
        self, contract_id: str, **overrides
    ) -> Dict[str, Any]:
        ...
        data = {
            ...
            "gdpr_compliance": max(
                0.0, min(1.0, base_score + random.uniform(-variance, variance))
            ),
            "employment_law_compliance": max(
                0.0, min(1.0, base_score + random.uniform(-variance, variance))
            ),
            "consumer_rights_compliance": max(
                0.0, min(1.0, base_score + random.uniform(-variance, variance))
            ),
            "commercial_terms_compliance": max(
                0.0, min(1.0, base_score + random.uniform(-variance, variance))
            ),
            "risk_score": (
                random.randint(1, 10) if base_score < 0.8 else random.randint(1, 5)
            ),
            "risk_factors": self._generate_risk_factors(base_score),
            "recommendations": self._generate_recommendations(base_score),
            "analysis_date": self.fake.date_time_between(
                start_date="-30d", end_date="now"
            ),
    ...
    def _generate_recommendations(self, compliance_score: float) -> List[str]:
        ...
        return random.sample(
            all_recommendations, min(num_recommendations, len(all_recommendations))
        )

    def generate_ai_generation_data(
        self, contract_id: str, **overrides
    ) -> Dict[str, Any]:
...
Date: ___________________        Date: ___________________
    ...
    def generate_audit_log_data(
        self, user_id: str, resource_type: str = "contract", **overrides
    ) -> Dict[str, Any]:
    ...
    def generate_bulk_test_data(
        self, company_id: str, user_id: str, count: int = 100
    ...
    def generate_realistic_business_scenario(
...
def bulk_test_data():
    ...
    return test_data_factories.generate_bulk_test_data(
        "test-company-id", "test-user-id", 50
    )
