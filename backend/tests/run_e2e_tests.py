...
class E2ETestRunner:
    ...
    def __init__(
        self, backend_path: str = "/Users/rezazeraat/Desktop/Pactoria-MVP/backend"
    ):
    ...
    def _run_pre_test_validation(self) -> Dict[str, Any]:
        ...
        validation_results = {"success": True, "checks": {}}
        ...
        try:
            ...
            if response.status_code == 200:
                validation_results["checks"]["server"] = {
                    "status": "running",
                    "success": True,
                }
            else:
    ...
    def _generate_final_report(
        ...
        # Aggregate results
        total_tests = sum(
            result.get("tests_run", 0) for result in category_results.values()
        )
        total_failures = sum(
            result.get("failures", 0) for result in category_results.values()
        )
        total_errors = sum(
            result.get("errors", 0) for result in category_results.values()
        )
        total_skipped = sum(
            result.get("skipped", 0) for result in category_results.values()
        )
    ...
    def _generate_performance_summary(
        ...
        for category, result in category_results.items():
            if "performance" in category:
                ...
                performance_data["benchmarks_met"][category] = result.get(
                    "success", False
                )
    ...
    def _validate_business_requirements(
        ...
        business_requirements = {
            ...
            "file_management": {
                "upload_download": "file_management_and_bulk_operations"
                in category_results,
                "bulk_operations": "file_management_and_bulk_operations"
                in category_results,
    ...
    def _generate_recommendations(self, category_results: Dict[str, Any]) -> List[str]:
            ...
            if not result.get("success", False):
                ...
                if failures > 0:
                    recommendations.append(
                        f"Address {failures} test failures in {category}"
                    )
                ...
                if result.get("duration", 0) > 300:  # 5 minutes
                    recommendations.append(
                        f"Optimize {category} test performance (taking {result.get('duration', 0):.1f}s)"
                    )
        ...
        if total_tests > 0:
            ...
            if success_rate < 95:
                recommendations.append(
                    f"Improve test success rate (currently {success_rate:.1f}%, target >95%)"
                )

        if not recommendations:
            recommendations.append(
                "All tests passing! Consider adding more edge cases and performance tests."
            )
    ...
    def print_final_report(self, report: Dict[str, Any]) -> None:
        ...
        for category, result in report["category_results"].items():
            ...
            print(
                f"   {status} {category.replace('_', ' ').title()}: {tests} tests in {duration:.1f}s"
            )
        ...
        if perf.get("benchmarks_met"):
            ...
            print(
                "   📊 Performance data will be available after running performance tests"
            )
        ...
        print(
            f"   Requirements Covered: {biz_val['covered_requirements']}/{biz_val['total_requirements']} ({biz_val['coverage_percentage']:.1f}%)"
        )
        ...
        if summary["overall_success"]:
            ...
            print(
                "🔧 Some tests failed. Please review the issues above before proceeding."
            )
...
def main():
    ...
    parser.add_argument(
        "--backend-path",
        default="/Users/rezazeraat/Desktop/Pactoria-MVP/backend",
