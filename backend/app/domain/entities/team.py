    ...
    def _validate_full_name(self, full_name: str):
        ...
        if not full_name or not full_name.strip():
            raise DomainValidationError(
                "Full name cannot be empty", "full_name", full_name
            )

        if len(full_name.strip()) < 2:
            raise DomainValidationError(
                "Full name too short (min 2 characters)", "full_name", full_name
            )

        if len(full_name.strip()) > 100:
            raise DomainValidationError(
                "Full name too long (max 100 characters)", "full_name", full_name
            )
...
class TeamInvitation(Entity[InvitationId]):
    ...
    def resend_invitation(self, days_to_extend: int = 7) -> str:
        ...
        if not self.is_valid():
            raise BusinessRuleViolationError(
                "Cannot resend invalid or expired invitation"
            )
    ...
    def _validate_invited_by(self, user_id: str):
        ...
        if not user_id or not user_id.strip():
            raise DomainValidationError(
                "Inviting user ID cannot be empty", "invited_by_user_id", user_id
            )
    ...
    def _validate_token(self, token: str):
        ...
        if not token or len(token) < 16:
            raise DomainValidationError(
                "Invalid invitation token", "invitation_token", token
            )
    ...
    def _validate_expiry(self, expires_at: datetime):
        ...
        if expires_at <= datetime.now(timezone.utc):
            raise DomainValidationError(
                "Expiry date must be in the future", "expires_at", expires_at
            )
...
class Team(AggregateRoot[str]):
    ...
    def invite_member(
        ...
        if projected_members > self._max_members:
            raise BusinessRuleViolationError(
                f"Team size limit exceeded. Maximum {self._max_members} members allowed"
            )
    ...
    def change_member_role(
        self, member_id: str, new_role: UserRole, changed_by_user_id: str
    ):
        ...
        if member.role == UserRole.ADMIN and new_role != UserRole.ADMIN:
            admin_count = len(
                [m for m in self.active_members if m.role == UserRole.ADMIN]
            )
            if admin_count <= 1:
    ...
    def _has_member_with_email(self, email: Email) -> bool:
        ...
        return any(
            member.email.value == email.value for member in self._members.values()
        )
