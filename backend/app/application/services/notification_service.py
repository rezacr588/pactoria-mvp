...
class NotificationApplicationService:
    ...
    async def send_notification(
        self, request: NotificationRequest, created_by_user_id: str
    ) -> str:
        """
        ...
        try:
            ...
            if request.related_entity_id:
                notification.set_related_entity(
                    request.related_entity_id, request.related_entity_type or "unknown"
                )
    ...
    async def send_bulk_notification(
        self, request: BulkNotificationRequest, created_by_user_id: str
    ) -> List[str]:
        """
        ...
        for user_id in request.recipient_user_ids:
            try:
                ...
                notification_id = await self.send_notification(
                    individual_request, created_by_user_id
                )
    ...
    async def get_user_notifications(
        ...
        return [
            self._notification_to_dict(notification) for notification in notifications
        ]
    ...
    async def mark_notification_as_read(
        ...
        if user_id not in recipient_user_ids:
            raise BusinessRuleViolationError(
                "User is not a recipient of this notification"
            )
    ...
    async def process_scheduled_notifications(self) -> int:
        ...
        for notification in notifications:
            try:
                ...
                self.logger.error(
                    f"Failed to process notification {notification.id.value}: {e}"
                )
    ...
    async def process_notification_retries(self) -> int:
        ...
        for notification in notifications:
            try:
                ...
                self.logger.error(
                    f"Failed to retry notification {notification.id.value}: {e}"
                )
    ...
    async def expire_old_notifications(self) -> int:
        ...
        for notification in notifications:
            try:
                ...
                self.logger.error(
                    f"Failed to expire notification {notification.id.value}: {e}"
                )
    ...
    async def get_notification_statistics(
        ...
        if stats.get("total_notifications", 0) > 0:
            stats["delivery_rate"] = (
                stats.get("delivered_notifications", 0) / stats["total_notifications"]
            ) * 100
            stats["read_rate"] = (
                stats.get("read_notifications", 0) / stats["total_notifications"]
            ) * 100
        else:
    ...
    async def notify_contract_status_change(
        self,
        contract_id: str,
        old_status: str,
        new_status: str,
    ...
    async def _process_notification_delivery(self, notification: Notification):
        ...
        for recipient in notification.recipients:
            ...
            channels = self._get_delivery_channels(
                notification.notification_type, recipient
            )
    ...
    def _get_delivery_channels(
        self, notification_type: NotificationType, recipient: NotificationRecipient
    ...
    async def _deliver_email(
        self, notification: Notification, recipient: NotificationRecipient
    ):
    ...
    async def _deliver_webhook(
        ...
        payload = {
            ...
            "recipient": {"user_id": recipient.user_id, "name": recipient.name},
            "subject": notification.get_personalized_subject(recipient),
            "content": notification.get_personalized_content(recipient),
