    ...
    **URL:** `ws://localhost:8000/api/v1/ws/connect?token=your_jwt_token`
    ...
    try:
        ...
        session_id = await websocket_manager.connect(
            websocket, current_user.id, current_user.company_id
        )

        logger.info(
            f"WebSocket connection established: user={current_user.id}, session={session_id}"
        )
...
async def get_websocket_stats(current_user: User = Depends(get_current_user)):
    ...
    **Admin Access Only:** This endpoint requires admin permissions
    ...
    return {"websocket_stats": stats, "generated_at": "2024-01-01T00:00:00Z"}
...
async def handle_websocket_message(
    websocket: WebSocket, message: Dict[str, Any], user: User
):
