...
class BaseDomainEventHandler(DomainEventHandler):
    ...
    async def handle(self, event: DomainEvent) -> None:
        ...
        try:
            self._logger.info(
                f"Handling event {event.event_type} for aggregate {event.aggregate_id}"
            )
            await self._handle_event(event)
            self._logger.info(f"Successfully handled event {event.event_type}")
        except Exception as e:
            self._logger.error(
                f"Failed to handle event {event.event_type}: {str(e)}", exc_info=True
            )
...
class NotificationEventHandler(BaseDomainEventHandler):
    ...
    async def _send_notification(
        self, message: str, recipients: list, notification_type: str = "info"
    ):
...
class AuditEventHandler(BaseDomainEventHandler):
    ...
    async def _create_audit_entry(
        self, event: DomainEvent, action: str, details: dict = None
    ):
